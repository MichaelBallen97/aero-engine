...
ten-row macOS pass is ✅ **PASS on ALL 10 ROWS** with every blank filled.**
...
**3.6.2 — Directional shadow map: COMPLETE IN CODE on `feat/3.6.2-directional-shadow-map` (ten
commits), NOT YET PUSHED, NOT YET REVIEWED, and PENDING its twelve-row macOS pass.** The renderer
...
`/editor`, `/tools`, `runtime/`, `vcpkg.json`, `cmake/` and `.github/` are byte-identical**, as are
...
`Plane` and the engine math types, all of which `aero_render` already had. `samples/phase-3-shadows`
...
Test inventory measured on `feat/3.6.2-directional-shadow-map` **(3.6.2 complete in code, on top of
`main` @ `64df342`)**, every number **re-measured on the tree in front of you, never derived by
addition and never carried forward from an earlier step or an earlier task** — read the totals from
doctest's own `filters:` line, never from a `grep -c` of case names. 3.6.2 moves `aero_tests`
**942 → 986** and nothing else; when it merges, re-measure rather than trusting that sentence.
**986, not 985**: the plan predicted 985 and `SM15` is the correction, added because seed `SH29`
otherwise reddened nothing.
...
**Doctest, SEVEN binaries: 986 / 1725 / 138 / 29 / 27 / 7 / 28.** 3.6.2 adds **+44** to `aero_tests`
alone (`SF1`–`SF28` tier-0, `SM1`–`SM15` behind `AERO_SHADER_TOOLS_ENABLED` in one new TU, and `SW1`
appended to the existing `rhi_device_test.cpp`) and leaves the other six **unmoved**; the shadow filter
lists **43** with tools on and **28** with them off, the `SM` tier absent by its gate. 3.6.1 had added
**+36** (`FC1`–`FC25` and `CD1`–`CD11`), and its culling filter still lists **25** tools-off. The tracked list read **five** until 3.1.5, because
...
the sample's `CMakeLists.txt` and `README.md` are not C-family) and **408 → 412** at 3.6.2 (+4:
`shadow.hpp`, `shadow.cpp`, `render_shadow_test.cpp` and `samples/phase-3-shadows/main.cpp` — the three
new `.hlsl` files are not C-family either), with
