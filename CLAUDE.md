...
**Phase 3 (Asset Pipeline & 3D Content) is OPEN, and EPIC 3.7 (Audio playback v0 · audio) is now
OPEN with 3.7.1 (Audio clip assets) as its first task — MERGED as PR #88 (merge commit `4892e65`,
nineteen commits plus the merge), all six CI checks green with `headSha == HEAD` asserted before the
merge, and macOS-validated ✅ PASS on ALL 11 ROWS (2026-08-23). It opens `engine/audio/`** (its
`.gitkeep` is gone, `engine/CMakeLists.txt` gains `add_subdirectory(audio)` **after** `assets`, and
`aero_audio`/`aero::audio` exist), **ships the
tree's FOURTH first-party binary format** (`.aerowave`, `docs/09`'s normative **§14**; Reserved
renumbers **§14 → §15**, so **§15 is the current Reserved section**), **takes `aero_cooker` to FIVE
subcommands** (`mesh`, `texture`, `skeleton`, `animation`, `audio` — both error literals, `--help`,
the README's frozen contract and the `run_case.cmake` arms all name five), and **freezes the
determinism manifest at 20 hash lines across FIVE arms / 40 cross-lane comparisons** with all 18
pre-existing hashes byte-identical and `ktx validate`'s 8 unchanged. **It makes NO SOUND** — playback,
components and spatialization are 3.7.2; this task exists to give 3.7.2 a clip to play. Details in the
3.7.1 block below and in full in `docs/10`.

**Epic 3.1 (AssetDatabase · assets) is CLOSED —
...
branch point in separate worktrees; 3.6.2 landed first, so 3.6.3 took `origin/main` in with a MERGE
(the 3.1.5 pattern, not a rebase) and **every whole-tree count on its page was re-derived from the
merged tree rather than carried forward**. Its twelve-row macOS pass is **RUN and COMPLETE
(2026-08-22): ✅ ALL 12 ROWS PASS** — row 2b only after the regression **its own control uncovered**
was fixed by PR #87 and the row re-run against the fix, on a `viewport_panel.cpp` byte-identical to
`main`'s. The first run's failure is kept on the page rather than overwritten. **The headline is
row 6**: at the linear value whose sRGB encoding is exactly 0.5, `--raw` reads **48**/255 and
`--tonemap=none` reads **116**/255 — a measured difference of 68 against a predicted 72, which dwarfs
the display-profile shift. Row 1's A/B is **measured, not judged**: against a purpose-built pre-3.6.3
worktree the cube's lit faces move **60 → 159** and **52 → 149** while the shadowed face moves only
10 → 35, the OETF's characteristic shape, and the brightest face reads **159 rather than a clipped
255**. Row 9: the pass costs **7.8 µs of a 16.7 ms editor tick** and `endFrame` exactly **doubles
(717 → 1430)**, which is structural proof the two-pass chain is live. Row 12: the
`render::PostProcess::resolve` zone is present at **17.07 µs mean over 472 samples**, and the export
confirms **this task adds NO plot**. **Three defects in the validation page itself are recorded on
it** rather than smoothed over: row 10's expected message is *structurally impossible* (it names
`res://fullscreen.vert` / `res://tonemap.frag`, but that string lives inside the
`#if AERO_SHADER_TOOLS_ENABLED` branch and is compiled out in the configuration the row tests);
**seed `T18` does NOT flip the picture** but maps the fullscreen triangle to NDC y ∈ {1, 5}, entirely
above clip space, so it renders **black and is indistinguishable from `T17`** — which matters because
row 1 lists it as the flip check, and row 1's real flip evidence is the sample's own printed legend
matching the capture; and **three of the six declared seeds (`T15`, `T16`, `T24`) are visually INERT
by derivation**, producing byte-identical output, which is a finding rather than a failure.**

**THE REGRESSION 3.6.3 SHIPPED, AND THE ROW THAT CAUGHT IT.** 3.6.3's code-review round closed a real
defect — a click on the viewport's overlay bar also fired a scene pick — with
`if (ImGui::IsAnyItemActive()) { pickArmed = false; }`. **That disabled viewport picking entirely**,
and it merged. `ImGui::Image` submits with **id 0**, so a click on the viewport image is *window empty
space* to ImGui, which responds by activating the window's move id (`imgui.cpp:5522` → `:5534` →
`SetActiveID(window->MoveId, window)` at `:5389`; `IsAnyItemActive()` is `g.ActiveId != 0` at
`:6617`) — so the condition was true on exactly the frames a pick was attempted. **The sentence that
licensed the guard is the part that was wrong**: its comment argued *"ImGui::Image submits with id 0
and never becomes Active"*, which is true about the **item** and irrelevant, because it is the
**window's `MoveId`** that goes active. **Fixed by PR #87 (merge commit `2c911e2`)**, all six checks
green: the overlay's claim on a press is now a **rect** recorded while drawing the interactive row and
consulted by a named `overlayOwnsPress()`, so only the row that actually has widgets claims a press,
an empty rect owns nothing (first frame, early returns, and the whole `-DAERO_SHADER_TOOLS=OFF`
build), and the original defect stays closed at the shared cause — the rect covers the gizmo bar's
buttons too, which had it since 2.3.3. **Not a revert.** Two lessons outlive it. **(1) An ImGui global
that answers a broader question than the one you are asking is not a guard** — `IsAnyItemActive()`
cannot tell "a widget took this click" from "the user clicked the window background". **(2) `I106(d)`
PASSED on the defective code**, because it pinned that a disarm line *exists* and sits after
`drawViewOptions()` — the INTENTION — rather than that a pick still *works* — the EFFECT; it was
**replaced, not patched**, and `I107` now drives real frames through `EditorApp::tick()` and asserts
ownership at coordinates derived from the **real** recorded rect, leading with an anti-vacuity block
because an empty rect would make every "does not own" assertion pass on a panel that recorded nothing.
**And the process lesson: row 2b's own assertion PASSED — it was its CONTROL that failed.** A
validation run that had checked only "the selection stayed stable" would have signed this regression
off.
...
pushed to **§14**, which task 3.7.1 then filled with `.aerowave` and pushed to **§15** — so **§15 is
the current Reserved section**); the **first mesh
...
**3.6.3 — Tonemap/gamma pass: MERGED as PR #86 (merge commit `e179285`, twelve own commits plus one
merge commit, cut from `main` @ `64df342`), CI-green on all six checks with `headSha == HEAD` asserted
before the merge, and macOS-validated (`editor/validation/3.6.3-tonemap-gamma.md`) — ✅ **PASS on ALL 12
ROWS**, row 2b only after the picking regression **its own CONTROL uncovered** was fixed by PR #87
(`2c911e2`) and the row re-run against the fix. CLOSES Epic 3.6 in code.** Sized S
...
**3.7.1 — Audio clip assets: EPIC 3.7 (Audio playback v0 · audio) OPENS with it. MERGED as PR #88
(merge commit `4892e65`, nineteen commits plus the merge, cut from `main` @ `03290bc` with no sibling
in flight, so it never merged `origin/main` and every count on it is a single-tree reading), all six
CI checks green with `headSha == HEAD` asserted before the merge, and macOS-validated
(`editor/validation/3.7.1-audio-clip-assets.md`) — ✅ **PASS on ALL 11 ROWS** (2026-08-23), row 11
recorded rather than judged by its own text.** Since 0.3.3 this engine could **open** an audio
device and write silence into it and had never been able to say what a sound *is*: `engine/audio/`
held one file, its `.gitkeep`. The editor had classified `.wav`/`.mp3`/`.ogg`/`.flac` as
`AssetKind::Audio` since 3.1.3 — so those files already got GUIDs, sidecars, cache entries and
hot-reload — and **nothing decoded one, nothing cooked one, no container held one and no runtime
type could be handed one**. Four layers move:
`/editor` decodes (miniaudio's dr_wav/dr_flac/dr_mp3, `stb_vorbis` for ogg — **both already present,
neither a new dependency**), `engine/assets` cooks, `tools/cooker` drives it, and **`engine/audio`
opens** with the one piece the texture column did not need — the runtime resource and its VFS loader.
**No dependency of any kind lands** and **no link line moves on any existing shipping target**:
`aero_editor_core` gains an *include directory*, never a link, because it already links
`aero::platform` PUBLIC and that archive already carries the one `MINIAUDIO_IMPLEMENTATION` unit. The
single link-line change in the branch is `aero::audio` joining **`aero_tests`**, a test target, and it
is recorded as such rather than glossed.

**THE PASS'S HEADLINE IS ROW 5.** The external anchor reads **0 differing bytes on BOTH wav and
flac** over its full 16 000 B, so dr_wav and dr_flac agree with **libavcodec** byte for byte. Row 4's
four duration deltas are **exactly 0.000000 s**, so the ±4608-frame tolerance is **entirely unused
slack**; row 2's `64 + 2 × ch × fr` identity holds 4/4 (**16 064 B** three times, **32 064 B** for the
stereo ogg) and `ffprobe` agrees with the header bytes 4/4 in row 3. Row 8 cooks a locally generated
3-minute stereo 44.1 kHz mp3 — **never committed** — in **0.16 s** at a **79.15 MB** peak into a
**31 752 064 B** artifact; row 9 loads it through a real `DirectoryBackend` in **10.5 ms** for a
resident delta of **31.87 MB** against 31.75 MB of artifact (**+0.36 %**), which is also a behavioural
witness that the buffer is **moved, not copied**, since a copy would read ≈63.5 MB. Row 10 is **0
unexpected lines across TEN samples** and row 1 **0 across four runs**. **Row 11 vindicates D12 a
second time**: mp3 agrees on Windows and Linux and **differs on macOS** (2/3), ogg **differs on all
three** (0/3) — **had either been frozen into the manifest, `cook-determinism` would be red.**

**FIVE FINDINGS, AND EVERY ONE IS A DEFECT IN THE PAGE RATHER THAN IN THE CODE.** (1) **Row 6's
stated input cannot witness A19**: `runAudio` hands `decodeAudioFile` the cook's own caps, so a
57.6 M-frame source is refused by the **decode's** pre-allocation check and `cookAudio` — where A19
lives — is never entered (the seeded peak moved 241.47 → 241.48 MB, sixteen kilobytes). **The row's
NUMBER was right and its INPUT was wrong**; on a corrected input — 4000 Hz stereo, which decodes fine
and is then refused by `cookAudio`'s rate bound — the difference is **241.19 → 356.19 MB = exactly
115.00 MB**, matching the prediction to two decimals, so **A19 is witnessed**. (2) **Row 6 is NOT
A23's coverage**, and A23 is unwitnessable by construction — the seeded build produced artifacts
byte-identical to correct output on all four fixtures, including against the **frozen**
`tone.aerowave`, at identical peak RSS. (3) **Row 7's command was VACUOUS on macOS**:
`ASAN_OPTIONS=detect_leaks=1` makes an ASan binary print *"detect_leaks is not supported on this
platform"* and **abort before `main`**, so the grep returned 0 because the program never ran — proven
with a `--version` control and re-confirmed independently. **Same species as `AD4`: a check that
returns the right answer for the wrong reason.** (4) **Row 7's mp3 expectation was wrong** — a
40 %-truncated mp3 is *accepted* (1775 frames), correctly, because **MP3 carries no completeness
claim**, and likewise a truncated wav; the battery was extended to **six inputs over three genuinely
different error paths**, all at 0 leak lines and 0 sanitizer lines. (5) Row 1's `^\[info\]` anchor
never matches this project's log format (a line starts with a timestamp), and **`timeout` does not
exist on macOS** — row 10 ran each sample under `perl -e 'alarm shift; exec @ARGV' 12`.

**AND THE LIMIT, STATED RATHER THAN GLOSSED: macOS has ASan but NOT LSan.** Row 7's zero proves the
refusals are **clean**, not that a leak is **absent** — and the one real leak this task found sits on
exactly those paths (stb_vorbis, 632 B, `leak:start_decoder`), visible only on Linux. **The Linux run
is what turns row 7 into evidence**, which makes this page's Windows and Linux rows matter more than
any other page's in the directory.

**Four rules from it that outlive the task.** (1) **`formatVersion == 1` MEANS s16 interleaved, and
FRAME-MAJOR IS NORMATIVE** (`docs/09` §14.2: frame *f*, channel *c* at `f * channels + c`). There is
no format code in the header, no `--format` flag, no AUTO rule and no planar layout — a one-value enum
is a shape that invites a second value, and the second would arrive without the cross-lane byte anchor
the first one has. **Adding f32 is a `formatVersion` bump; bundle it with loop points, never ship
either alone.** (2) **ZERO PADDING SITES, and that is a contract.** One bulk region and it is LAST, so
`totalBytes == 64 + 2 × channels × frameCount` **exactly**, with no rounding term anywhere — the
opposite end of the spectrum from `.aeroanim`'s single checked site. The 64-byte header being
16-aligned **does not license a typed span** over the samples; every read goes through `getU16`.
(3) **THIS IS THE FIRST COOK IN THE SUBSYSTEM THAT CARRIES NO FLOATING POINT AT ALL** — not "float
data moved bit for bit" as `.aeromesh` and `.aeroanim` are, *absent*. **INV-A1's scope is
`cooked_audio.{hpp,cpp}` + `audio_cook.{hpp,cpp}`**, which is exactly the scope that is true;
**INV-T4 was mis-stated once already as a claim about `engine/assets` as a whole**, which is and
always was false. State the scope, never the subsystem. (4) **`ditherMode` is set explicitly to
`ma_dither_mode_none` and is SOURCE-TEXT PINNED**, with `ma_decoder_config_init(ma_format_s16, 0, 0)`'s
two "keep the stream's own" sentinels pinned beside it: **dither is randomised by construction**, so a
dithered f32→s16 conversion produces different bytes **on consecutive runs of the same binary on the
same machine**, and the wav/flac manifest lines would flap with no code change at all.

**DETERMINISM IS CLAIMED FOR WAV AND FLAC AND NORMATIVELY REFUSED FOR MP3 AND OGG** (`docs/09` §14.7,
the §13.7 pattern). dr_wav and dr_flac are integer decoders and enter the frozen manifest; dr_mp3 and
stb_vorbis run floating-point transforms whose paths differ by SIMD availability and FMA contraction
policy, and **neither may ever enter it**. **The refusal is MEASURED, not asserted**:
`cooker.audio_lossy_digests` prints both digests on every lane on every push and asserts exit 0, a
non-empty artifact and a literal run count of 2 — enough that a broken audio path reddens it — while
**never asserting a digest value**, so nobody can "finish" it by pasting the hashes in. **Whatever the
three-lane reading says, the two stay out**: agreement today at one vcpkg baseline and three pinned
compilers is a *fact*, not a *contract*.

**THE EXTERNAL ANCHOR, and it is what makes this format different from the other three.** `.aerowave`
is first-party so no third-party validator will open one — but **its sample region is not first-party
at all**: raw interleaved s16 LE is the most widely implemented byte layout in audio.
`tests/fixtures/audio/tone.s16le.pcm` is **ffmpeg's own decode** of the wav fixture (exactly 16 000 B,
committed), and both the decode and the cook are asserted byte-identical to it. That is dr_wav and
dr_flac checked against **libavcodec** — the answer to 3.3.3's sharpest recorded sentence, *"NO TEXTURE
LINE HAS AN EQUIVALENT TIE."* **If it ever reddens, that is a FINDING and the anchor is doing its job:
understand the difference and record it, NEVER regenerate the golden from our own output.**

**The fixture set is 1.0 s because 0.25 s silently lost half the ogg.** 2000 frames is shorter than
Vorbis's 2048-sample long block, so the encoder emitted one packet and the file decoded back to
**1024 frames — 976 lost, ~49 %**. All four candidate lengths were measured before choosing; **1.0 s
is the shortest at which the Vorbis round trip is frame-exact**, so all four encodings decode to the
same 8000-frame signal and every number in the task comes off one arithmetic. **Mono ogg is impossible
with this toolchain** (the native experimental Vorbis encoder refuses anything but stereo, and there
is no `oggenc`/`flac`/`sox` here) — **which is an upside**: with one channel frame-major and
channel-major are the same byte order, so `tone.ogg` is the **only** committed source whose decode can
witness a channel-order defect at all. **Any row comparing an ogg reading against 1 channel is
comparing against the wrong number.** And **the measured mp3 frame-count delta is ZERO** — dr_mp3
reproduced ffmpeg's 8000 exactly, so the ±4608-frame (four-granule) tolerance is **untested slack**
rather than slack being consumed.

**Its 34-seed matrix ran to completion with NO GAP FOUND** — every seed either reddened a case or
failed to compile — and A19, A23 and A28 were flagged expected-declared before it ran. **All three
have since been RESOLVED, and the declared class is ONE seed — by a wide margin the smallest in the
project** (3.5.1 declared four, 3.5.2 three, 3.6.1 three, 3.6.3 nine). **A28** (dropping the
**in-loop** cap check) was closed by the code-review round, which built the lying-length ogg its own
justification had called unaffordable: the lie points **downward**, so it costs **six bytes** rather
than the ~55 MB that justification assumed, and `AD21` reddens alone under it. **A19** (moving the cap
checks below the buffer resize — identical status, message and empty `bytes`, only the **peak
allocation during a refusal** differs) was closed by the macOS pass, once row 6's INPUT was corrected.
**A23 is the survivor** (dropping the output's zero-initialisation), and it is unwitnessable **BY
CONSTRUCTION** rather than by luck: `std::vector<std::byte>(n)` value-initialises **by definition**,
so the `0xCD` seed is not spellable without changing `AudioCookResult::bytes`' type. **It stays
declared and the zero-init stays** — defence for a field a *future* `formatVersion` forgets to write,
and **a defence is not deleted because no seed can reach it.** **This task ran no shader, GPU or
sample seed, because it has none of those things.** Two plan witness attributions were **corrected
by measurement**: `A1` was predicted to COMPILE and does not (a second `static_assert`,
`H_TOTAL_BYTES + 8 == COOKED_AUDIO_HEADER_BYTES`, which the plan's "48 % 16 == 0" reasoning missed —
**four seeds are build failures where the plan predicted two**), and **`A14` as written does NOT
fail to build**: replacing the derived sample cap
with its own **correct** literal satisfies the assertion, verified with a standalone probe, so **that
`static_assert` is a DRIFT tripwire rather than a literal-substitution tripwire** and `CA4` is what
witnesses the substitution itself.

**IT SHIPPED A LEAK CI CAUGHT AND NO LOCAL RUN COULD SEE.** macOS, Windows, lint and vcpkg-baseline
all passed `96766aa`; **Linux failed, and it was never flake.** `aero_editor_shell_test` reported
`1743 passed | 0 failed` and `301543 assertions | 0 failed | Status: SUCCESS!`, and **LeakSanitizer
then fired at teardown** — 632 B in one allocation at `stb_vorbis.c:3740`, reached from `AD14`'s
truncated `.ogg`. **The defect is upstream and the reading was verified against the pinned source**:
`start_decoder`'s **sparse** arm puts the codeword-length scratch in a **local only**, while the
non-sparse arm also parks it on the codebook where `vorbis_deinit` could reach it; with
`alloc_buffer == NULL` (what we pass, correctly) `setup_temp_malloc` is a plain `malloc`, and four
error returns sit between it and the first matching `setup_temp_free`. **Nothing of ours can close
it**: `stb_vorbis_open_memory` returns `nullptr` there, so `decodeWithStbVorbis`'s `unique_ptr` guard
is **never constructed** — there is no handle to own. Closed with one **frame-scoped, evidence-gated**
`tests/lsan.supp` entry (`leak:start_decoder`), 3.2.5's assimp precedent exactly; **never module-wide**,
because `leak:stb_vorbis` would blind LSan to a first-party leak allocated anywhere inside the library
— and nothing of ours allocates inside `start_decoder`. **The port is not patched** (this tree does not
patch third-party sources) and **the fixture stays exactly as it is: `AD14` is the coverage, not the
bug**. `ci.yml` needed no change — the Debug ctest step has pointed `LSAN_OPTIONS` at that file since
0.4.2, and the same run's `Suppressions used:` table proves the wiring live. **The lesson, learned a
second time (3.2.5 was the first, and this is the first time a DECODER rather than an IMPORTER was the
source): a leak in a third-party error path is invisible on two of the three lanes.**

**Three traps found, each worth carrying.** (1) **`git grep -- $F` is SILENTLY VACUOUS under zsh** — a
pathspec list in a shell variable does not word-split, so the command searches one bogus pathspec,
matches nothing, exits 1, and a `&& … || echo "none (ok)"` idiom **reports a clean result on a dirty
tree**. Caught at the gate because AC-3's own "read, do not count" grep came back empty when the plan
predicted matches — **the prediction is what exposed it**. Use `${=F}` or literal paths, and check
every guard grep in **both** directions. Same species as 3.1.5's `->Data` grep and the POSIX `\b`
degradation: **a guard command that cannot fail is worse than no guard command.** (2) **`editor/CMakeLists.txt`
may not name a not-yet-existing `src/*.cpp` even in a COMMENT** — `AI2`/`AI3` derive the editor's TU
list by scanning that file's **raw text** and then `REQUIRE` every name to be a readable file, so a
prose mention of a source that does not exist yet turns them red for a reason that is not a violation.
(3) **`AD4` is a TAUTOLOGY BY CONSTRUCTION** — it asserts
`isCookableAudioName(n) == (audioSourceFormatForName(n) != Unknown)` while the implementation
*composes* the first from the second, so it can never fail. **`AD5`'s four literals are the real pin.**
It is kept rather than deleted, because it documents the intended relationship and would catch an
implementation that stopped composing them — but it is **not** a witness for the extension table.

**One cost accepted and named rather than hidden.** Removing `MA_NO_DECODING` puts dr_wav, dr_flac and
dr_mp3 into every binary that links `aero_platform`, which at Phase 5 includes the runtime. The
expectation is that `--gc-sections` / `/OPT:REF` strip them, since nothing there calls a decoder —
**and that expectation is UNVERIFIED.** `#define MA_API static` is the recorded escape hatch;
**owner is Phase 5's packager.** This matters because a runtime that *can* decode an mp3 is a runtime
that can be *handed* one, and ADR-008 says it is handed a `.pak` of cooked artifacts — the decode
living in `/editor` for **all four** formats is what keeps that true at the source level, and the
archive contents are the half still open.

**Named handoffs.** Play/stop/volume/pitch, `AudioSource`/`AudioListener`, spatialization, what a
**positioned stereo** source means, and `samples/phase-3-audio` → **3.7.2** (Phase 3's gate word is
*audible*, and nothing here can produce a sound). The `engine/audio` public-header guard → **3.7.3**:
**the link line makes it green TODAY by construction; a guard proves it STAYS green**, which is a
different job, and **3.7.2 is the task that can most easily void it**. Streaming / partial residency →
**unowned**, trigger the first clip that will not fit. Loop points (`smpl`, `LOOPSTART`) → **unowned**,
a `formatVersion` bump and **never alone**. f32 or compressed sample formats → **unowned**, bundle with
loop points. An audio readout or waveform thumbnail → **unowned** (this task adds **no editor UI at
all**). Editor-side cook-on-import → **unowned, shared with textures**. Buses, mixers, effects →
**6.4.1 / 6.4.2**. The runtime's decoder dead-weight → **Phase 5 packager**.
...
→ **`assets`** → **`audio`** → `platform` → `rhi` → `render` → `reflect` → `scene` → `scene_render` →
...
**`engine/audio/` OPENED at task 3.7.1** and its `.gitkeep` (dated 13 July 2026) is gone;
`engine/CMakeLists.txt` gains **one line**, `add_subdirectory(audio)`, placed **after**
`add_subdirectory(assets)` so the `aero::assets` alias exists. It holds the **runtime resource**, not
the format — one pair, `clip.{hpp,cpp}` (`AudioClip`, `AudioClipLoadStatus`,
`audioClipLoadStatusLabel`, `AudioClipLoadResult`, `loadAudioClip`) plus the `audio.hpp` umbrella.
**`aero_audio` links `aero::core` + `aero::assets` PUBLIC and `aero::profiling` PRIVATE and NO vcpkg
package, ever** — which makes it the **SECOND** target in this tree (after `aero_assets`) whose
...
ERROR rather than a guard finding**, and the CMakeLists carries that warning in its own header.
**Adding a `find_package` there voids it silently while CI stays green. 3.7.3 is going to be asked to
guard exactly this, and 3.7.2 is the task that can most easily void it.** `AudioClip` is **move-only
with copy deleted** and **holds no span of anyone else's memory**; a container wanting clips by value
stores them by `unique_ptr` or by index, as every other resource here already does.
...
consumer in question is the thing being built. It holds the cooked-asset formats and nothing else — **eleven pairs since task
3.7.1**: `cooked_mesh.{hpp,cpp}`, `mesh_cook.{hpp,cpp}`, `cooked_texture.{hpp,cpp}`,
`texture_cook.{hpp,cpp}`, `bc_block.{hpp,cpp}`, `cooked_skeleton.{hpp,cpp}`,
`skeleton_cook.{hpp,cpp}`, `cooked_animation.{hpp,cpp}`, `animation_cook.{hpp,cpp}`,
`cooked_audio.{hpp,cpp}` and `audio_cook.{hpp,cpp}`; it links
...
**Task 3.7.1 touches THREE engine subsystems, OPENS one of them, and adds THREE pairs**:
`engine/audio/{CMakeLists.txt, include/aero/audio/{clip.hpp, audio.hpp}, src/clip.cpp}` (the layer
opening, `.gitkeep` deleted), `engine/assets/{cooked_audio,audio_cook}.{hpp,cpp}` (the subdirectory's
third growth since 3.3.2; `engine/assets/CMakeLists.txt` gains two source lines with its
`target_link_libraries` block byte-identical), and `engine/platform/src/miniaudio_impl.c` losing
**one define** — `MA_NO_DECODING` and its comment, with the other five trims unchanged.
`engine/CMakeLists.txt` gains one line. `/editor` gains **TWO pairs, taking the count to TWENTY-SIX**
(re-counted at task end, never added to the remembered number): `audio_cook_source.{hpp,cpp}` (PUBLIC
and PURE — the name predicates; its header includes **`<cstdint>` and `<string_view>` and nothing
else, forever**, the `import_settings.hpp` rule's second application) and `audio_decode.{hpp,cpp}`
(the **only** `<miniaudio.h>` TU outside `engine/platform/src/` and the **only** `<stb_vorbis.c>` TU
anywhere in the tree, both verified by grep at the gate). `editor/CMakeLists.txt` gains one
`find_path`, one `SYSTEM PRIVATE` include directory and two source lines, and **no link line**.
`tools/cooker/src/main.cpp` gains `Subcommand::Audio` and `runAudio`, written **above `runTexture`**
because `cooker.texture_nothing_written_on_failure` pins a source-text ordering inside the region
between `runTexture` and `runMain` and requires `writeTextFileAtomic(args.outputPath` to occur exactly
once in it. **`engine/core`, `engine/platform/include`, `engine/platform`'s other three sources and
its `CMakeLists.txt`, `engine/rhi`, `engine/render`, `engine/reflect`, `engine/scene`,
`engine/scene_render`, `engine/scene_serialize`, all nine pre-existing `engine/assets` pairs, every
`editor/src/*.cpp` and `editor/include/**` other than this task's two pairs, `editor/third_party/`,
`runtime/`, `shaders/`, `samples/`, `cmake/`, `tools/shaderc/`, `tools/reflect-gen/`,
`tools/cooker/CMakeLists.txt`, `vcpkg.json`, the `/vcpkg` submodule pin,
`tests/platform_boundary_probe.cpp` and `docs/tasks/phase-3.md` are byte-identical** — verified path
by path at the gate rather than asserted. **No dependency of any kind lands.**

**Two byte-identity claims are AMENDED, and both were unsatisfiable as the spec wrote them.**
**`.github/workflows/ci.yml` is NOT byte-identical** (AC-21 amended): the manifest's totals are
literals in **four** files, not one, and `ci.yml` holds five of them — `18 → 20` at four sites,
`36 → 40` at two, four upload paths to five, and three prose sentences. This is 3.5.2's contradiction
repeated, resolved up front this time. And **`check-platform-boundary.sh` differs by exactly TWO `#`
comment blocks and no executable line** (AC-20 amended — the spec said "one prose sentence"; the
miniaudio-confinement claim appears **five** times, two in comments and **three in printed strings**).
**The three printed strings stay byte-identical, deliberately**: they are advice handed to somebody
who has just tripped a guard whose scan set is `engine/*/include/*`, and for that reader the sentence
stays exactly true — editing them would also change the guard's **output**, which is what the "no
logic change" clause protects. **Verified the strong way**: `main`'s copy of the script and the
branch's copy were run **against the same tree** and their output is byte-identical, which the plan's
`git stash` form could not have shown because it varies the tree as well as the script.
`.github/scripts/` is byte-identical apart from that one file, and **the guard's logic genuinely does
not need to change** — `HEADER_GLOB` is `engine/*/include/*`, so `editor/src/audio_decode.cpp` is
outside the scan set entirely, and self-test 1b derives its subsystem set from the tree, so `audio`
joins both sides automatically the moment its headers are tracked. **Nothing in the script had to
learn the new subsystem's name**, which is exactly why that self-test was written to derive rather
than to enumerate. Its OK banner's scanned count moves **73 → 77** — **+4, not the plan's predicted
+2**, because the two new `engine/assets` public headers enter that glob as well as the two
`engine/audio` ones.

Test inventory measured on `feat/3.7.1-audio-clip-assets`, which **never merged `origin/main`** (no
sibling task was in flight), so every figure below is a single-tree reading against a `main` @
`03290bc` baseline that was itself measured before the first commit. Every number **re-measured on
the tree in front of you, never derived by addition and never carried forward from an earlier step or
an earlier task** — read the totals from doctest's own `filters:` line, never from a `grep -c` of case
names. **This is 3.1.5's recorded lesson**: the moment a branch merges `origin/main`, every whole-tree
count on its own page goes stale, and adding one task's delta to another task's baseline is exactly
the arithmetic that produces a confident wrong number.

**The baseline itself corrected a remembered number.** `aero_editor_imgui_test` was recorded at
**142** after 3.6.3, but PR #87 replaced `I106(d)` with `I107` and the commit message could not settle
whether that was a rename or an addition; **measured at the branch point it reads 143**, so it was an
addition. **That is why the baseline is measured rather than carried**, and it is the only figure of
the seven that moved between 3.6.3's page and this one.

**`ctest -N` reads 168 / 76 / 89** — up from 157 / 65 / 78, **+11 and IDENTICAL in all three**, which
is the lockstep the cooker's lack of a gate flag requires. The eleven are the nine `cooker.audio_*`
arms (`happy`, `unknown_flag`, `missing_input`, `bad_extension`, `output_dir_missing`,
`nothing_written_on_failure`, `guid_written`, `determinism`, `all_four_formats`) plus
`audio_golden_manifest` plus `audio_lossy_digests`; the cooker arm count reads **70 in all three**.
**The four new engine and editor test TUs move `ctest -N` not at all**, because `aero_tests` and
`aero_editor_shell_test` each register as a single ctest entry — their growth reads only in doctest.
**A SMALLER move in a reduced configuration would mean the cooker block had accidentally grown a
gate, and no test can report that**, which is why it is a gate step rather than a case.

**Doctest, SEVEN binaries: 1088 / 1746 / 143 / 29 / 27 / 7 / 28**, against a measured baseline of
1028 / 1725 / **143** / 29 / 27 / 7 / 28 — so **`aero_tests` +60** (`CA1`–`CA26` on the container and
its parser, `AK1`–`AK20` on the cook, `AC1`–`AC14` on the clip and its loader, across three new TUs),
**`aero_editor_shell_test` +21** (`AD1`–`AD21`, one new TU — `AD19`/`AD20`/`AD21` added by the
code-review round) and **the other five +0**: this task adds no ImGui case, no component, no
serializer, no reflected field and no `reflect-gen` growth.
**The tools-OFF reading is the one that matters and it moves by the same +21 / +0**: measured in a
fresh `-G Ninja` reduced build **that was configured, built and RUN for `aero_editor_shell_test`
specifically** (1722 total there), `AD*` lists **21** and all 21 pass, and the three `aero_tests`
filters list the same 26 / 20 / 14 they do with tools on. `tests/editor/audio_decode_test.cpp` joins its target
**unconditionally and contains no `#if` of any kind** — `git grep -c '#if'` on it returns nothing.
**That check exists because of 3.6.3**, whose four new editor cases first landed inside a file-level
`#if AERO_SHADER_TOOLS_ENABLED` block with everything green while the one arm that mattered never
ran: **a suite that omits a case passes just as loudly as one that runs it.**

**Why the triple behaves the way it does — the reason matters more than the number, and it is
unchanged since 3.1.5.** The three configurations are tools ON, then
`-DAERO_REFLECT_TOOLS=OFF -DAERO_SHADER_TOOLS=OFF`, then `-DAERO_REFLECT_TOOLS=OFF` alone.
`aero_tests`, `aero_editor_shell_test` and
`aero_editor_imgui_test` each register with ctest as a **single entry** (`tests/CMakeLists.txt`), so
3.4.1's 57 new doctest cases, 3.4.2's 84, 3.5.1's 74, 3.5.2's 100, 3.1.5's **190**, 3.6.1's **36**,
3.6.3's **46** and 3.7.1's **81** move it not at all, and samples register no test. **Until 3.1.5 the triple moved only for `cooker.*` cases**
(**117 → 131 → 133 → 144 → 154 → 168**, **28 → 42 → 44 → 55 → 65 → 76**,
**41 → 55 → 57 → 68 → 78 → 89**), because
...
**How the doctest totals got here, per contributing task — deltas, NEVER a sum.** The current reading
is the seven-binary line above (**1088 / 1746 / 143 / 29 / 27 / 7 / 28**), measured on this tree; the
figures below are how each task moved it, kept so an older page can be read against the right
baseline. **3.6.2 and 3.6.3 together produced 1028 / 1725 / 142 / 29 / 27 / 7 / 28 on their merged
tree** — and the `aero_editor_imgui_test` figure of **142** was superseded by PR #87, which took it to
**143**; that correction was found by measuring the branch point rather than by trusting the number.
3.6.2 added **+44** to
...
read 142 tools-on and 128 tools-off then, and reads 143 / 129 now** — and that second number is a real
check rather than a decoration: 3.6.3's four editor cases first landed inside 3.1.5's file-level
...
configuration.
`aero_editor_core` sources **65 → 72** and tracked `editor/src/*.cpp` **66 → 73** (3.1.5's seven
pairs), then **73 → 75** at 3.7.1 (`audio_decode.cpp`, `audio_cook_source.cpp`); tracked
`editor/src/*.hpp` stays at **23**, because 3.7.1 adds **no src-private editor header**.
...
and `samples/phase-3-tonemap/main.cpp` — its two `.hlsl` files are not C-family either) and
**419 → 434** at 3.7.1 (**+15**: 3 in `engine/audio` — `clip.hpp`, `audio.hpp`, `clip.cpp` — 4 in
`engine/assets`, 4 in `/editor` and 4 in `tests/`; **`engine/audio/CMakeLists.txt`,
`tests/fixtures/audio/README.md` and the six binary fixtures are NOT C-family and are not counted**),
with `check-project-no-delete.sh`'s Check B scan **66 → 73** at 3.1.5, **unmoved at 73** through
3.6.1, 3.6.2 and 3.6.3 — none of those three adds an `editor/src/*.cpp` — and **73 → 75** at 3.7.1.
All re-measured **after `git add`**, since `git ls-files` sees
only tracked files, and both globs picking the new files up automatically — **neither script changes,
and `.github/scripts/` is byte-identical to `main` apart from `check-platform-boundary.sh`'s two `#`
comment blocks, whose OUTPUT is byte-identical (verified by running both versions against the same
tree).** Guard count stays **six**; Check A's six-file denylist and Check B's
two-file `PERMITTED_DELETERS` are unchanged in membership, and **neither of 3.7.1's two new editor TUs
is in either** (nor were 3.1.5's seven), which is what makes a future destructive call in one of them
a hard CI failure.
...
`tests/fixtures/assets/skinned.gltf`. **The determinism manifest is now FROZEN at 20 lines across FIVE
arms / 40 cross-lane comparisons** (18 / four / 36 from 3.5.2 until 3.7.1 added the `audio-` arm),
with all 18 pre-3.7.1 hashes byte-identical and `ktx validate`'s 8
unchanged. **3.7.1 commits the tree's first `.aerowave`** — `tests/fixtures/audio/tone.aerowave`
(**exactly 16064 B** = `64 + 2 × 1 × 8000`), cooked once by the real binary under the tree's standard
all-nonzero test GUID and **frozen** — beside four encodings of one self-authored 1.0 s signal
(`tone.wav`, `tone.flac`, `tone.mp3`, `tone.ogg`) and **the external anchor**, `tone.s16le.pcm`
(**exactly 16000 B**, ffmpeg's own decode, raw interleaved s16 LE, no container). **Only the wav and
flac artifacts enter the manifest**; mp3 and ogg are measured by `cooker.audio_lossy_digests` and
**normatively never frozen** (`docs/09` §14.7). **3.1.5 adds no manifest arm and no cooked artifact at all** — it commits one new fixture,
...
`toupper('0') == '0'`.
`git grep -nE '_WIN32|__APPLE__|__linux__' -- engine/assets engine/audio engine/render engine/scene tools/cooker`
reads **zero lines** — `engine/audio` joins that list at 3.7.1, and the failure mode it exists to
prevent is a decode that succeeds on one lane and fails on another — and the
...
never a use. **FOUR gate greps are NOT literally zero and must be READ rather than counted**:
`git grep -n 'find_package' -- engine/assets/` returns two comment lines stating the prohibition,
**`git grep -n 'find_package' -- engine/audio/` returns two more of the same** (3.7.1),
the `tools/` process-spawn grep matches "libsdl-org fork" twice in `tools/shaderc/README.md`, and
**INV-A1's float grep over the four audio cook/container files returns eleven matches of which only
three are code** — `cookedAudioDurationSeconds`'s two lines and `AudioCookStats::durationSeconds`,
every one reader-side or a reported statistic, the rest prose stating the prohibition.

**AND A GREP THAT LOOKS FINE CAN BE SILENTLY VACUOUS.** `git grep -- $F` with a pathspec list held in
a shell variable **does not word-split under zsh**: it searches one bogus pathspec, matches nothing,
exits 1, and a `&& … || echo "none (ok)"` idiom **reports a clean result on a dirty tree**. Found at
3.7.1's gate, because the INV-A1 grep above came back *empty* when the plan predicted matches — **the
prediction is what exposed it.** Use `${=F}` or literal paths, and **check every guard grep in BOTH
directions**. Same species as 3.1.5's `->Data` prefix trap and the POSIX `\b` degradation on BSD:
**a guard command that cannot fail is worse than no guard command.**
