...
either phantom shape. Mechanical gate green: 95/95 on both macOS presets, both reduced configurations
rebuilt fresh at **1199** cases each with `OI1` present in both, six guards passing byte-identical to
`main` (`.github/scripts/` untouched, unlike 3.2.2), clang-format/clang-tidy clean by exit code. **The
32-seed sabotage matrix ran to completion**: 24 seeds matched their prediction exactly, one genuine
coverage gap found and closed with a new case (`OI81`), three confirmed non-discriminators matching a
prior finding, one (`err` never written by `LoadMtl`) PROVEN a non-discriminator by direct source
reading rather than merely failing to find a counterexample, and two revealed a minor, recorded
prediction imprecision with no real coverage hole. **R5 closed with a measurement, and the result is a
real surprise recorded rather than smoothed over**: at ~150 MB, the Structure-depth SCAN
(~107–113 ms warm) costs roughly **3x** the file READ (~34–46 ms) — the scan dominates, the opposite of
the shape the plan predicted — while the steady-state scan still costs ~0 (D15/INV-C5 unaffected).
**The mechanical gate, sabotage matrix and documentation are done; the PR, CI run and macOS validation
pass are NOT** — reserved for the merge step. Zero paths under `engine/` — the no-engine-change streak
reaches **five**. Full detail — every §A correction, both build-time findings, the graded 32-seed
matrix and the measured inventory — is in `docs/10-engineering-log.md`'s 3.2.3 entry.
