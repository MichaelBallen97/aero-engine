...
OPEN with 3.7.1 (Audio clip assets) as its first task — complete in code on
`feat/3.7.1-audio-clip-assets`, PR #88, the full local gate green and the eleven-row macOS pass
PENDING. It opens `engine/audio/`** (its `.gitkeep` is gone, `engine/CMakeLists.txt` gains
`add_subdirectory(audio)` **after** `assets`, and `aero_audio`/`aero::audio` exist), **ships the
...
**3.7.1 — Audio clip assets: EPIC 3.7 (Audio playback v0 · audio) OPENS with it. Complete in code on
`feat/3.7.1-audio-clip-assets` (cut from `main` @ `03290bc`, ten commits, no sibling in flight so it
never merged `origin/main` and every count on it is a single-tree reading), PR #88 open, the full
local gate green, and the eleven-row macOS pass PENDING** (`editor/validation/3.7.1-audio-clip-assets.md`,
written before the pass as always). Since 0.3.3 this engine could **open** an audio device and write
silence into it and had never been able to say what a sound *is*: `engine/audio/` held one file, its
`.gitkeep`. The editor had classified `.wav`/`.mp3`/`.ogg`/`.flac` as `AssetKind::Audio` since 3.1.3 —
so those files already got GUIDs, sidecars, cache entries and hot-reload — and **nothing decoded one,
nothing cooked one, no container held one and no runtime type could be handed one**. Four layers move:
...
failed to compile — and **A19, A23 and A28 are CONFIRMED DECLARED**, flagged before the matrix ran.
**The declared class is three seeds, the smallest since 3.6.1**, and every one is about a *cost* or an
*unallocatable input* rather than an output nobody can see: A19 moves the cap checks below the buffer
resize (identical status, message and empty `bytes`; only the **peak allocation during a refusal**
differs), A23 drops the output's zero-initialisation (**declared, not closed** — the writer covers
every byte today, so it is defence for a field a *future* version forgets to write, and **a defence is
not deleted because a seed could not see it**), and A28 removes the **in-loop** cap check while
keeping the pre-allocation one (unreachable from any honest fixture; seeing it needs a lying-length
source, ~55 MB of PCM at cap scale). **Validation rows 6 and 7 are their only coverage anywhere —
rows 6 and 6 and 7 respectively, A23 sharing A19's row because both are about what a refusal COSTS
rather than what it RETURNS.** **This task ran no shader, GPU or sample seed, because it has none of
those things.** Two plan witness attributions were **corrected by measurement**: `A1` was predicted to
COMPILE and does not (a second `static_assert`, `H_TOTAL_BYTES + 8 == COOKED_AUDIO_HEADER_BYTES`,
which the plan's "48 % 16 == 0" reasoning missed — **four seeds are build failures where the plan
predicted two**), and **`A14` as written does NOT fail to build**: replacing the derived sample cap
