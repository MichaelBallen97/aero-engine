...
| **Next task** | **~~Finish 3.3.2~~ DONE, and fully validated on macOS** — merged as PR #76 (`cf8575a`) with `headSha == HEAD` asserted, and macOS-validated ✅ on **every row** (2026-08-14). R2 (BC1 PSNR, worst deficit **0.85 dB**, ours AHEAD on one image), R3 (**0.68–0.71 s at 181.3 MB peak** for a 4096² BC3 cook, byte-identical across three runs, and NOT importer-dominated unlike 3.3.1's R7) and **R1 (all eight artifacts pass `ktx validate` 4.4.2, non-vacuity proven by re-seeding the C1 regression and watching the validator reject it)** are all closed. **No macOS debt remains on this task.** Next: **(a) 3.1.5 (drag-into-scene)**, fully unblocked — it depends on 3.1.3 and 3.2.1 (both merged), all eight importable extensions work, and both cooks now give it artifacts to place. **3.1.5 owns three decisions**: sub-asset identity (3.2.1's D13), replacing `LOCAL_MESH_HALF_EXTENT` (2.3.1's knowingly-wrong constant), and **the node-hierarchy gap 3.3.1 named but does not own**. **(b) 3.3.3 (cook determinism golden test)** is unblocked and now has **both** cook kinds to compare — it is the task that turns cross-platform byte-identity into a CI job, which is what the no-floating-point design exists for, and `ktx validate` is now a proven, sudo-free tool it can call. **(c) 3.4.1** owns the `rhi::TextureFormat` block-format contract change 3.3.2 names, and nothing can upload a cooked texture until it lands. And **(d) the seven ticked-but-unmeasured validation rows**, plus the four-phase Windows/Linux platform-validation debt. See `docs/tasks/phase-3.md`. |
