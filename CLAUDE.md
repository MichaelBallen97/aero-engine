...
| **Phase 2** — Editor | Epic 2.1 (ImGui integration) **CLOSED in code**, macOS human-validated PASS; Windows/Linux rows pending (`editor/VALIDATION.md`). Epic 2.2 (Core panels) **CLOSED in code** — 2.2.1 Hierarchy, 2.2.2 Inspector, 2.2.3 Viewport, 2.2.4 Asset browser and 2.2.5 Log/console panel all landed; **no `PlaceholderPanel` remains**. 2.2.1–2.2.4 are macOS human-validated PASS with Windows/Linux rows pending; **2.2.5 is macOS human-validated PARTIAL — 11 of 15 rows PASS, 4 BLOCKED because the editor emits no log record after startup**, so they cannot be run at all. The gate stays open pending a triggerable runtime log source and a non-macOS pass (`editor/VALIDATION.md`). Epic 2.3 (Manipulation) **in progress** — 2.3.1 Editor camera **CLOSED in code and macOS human-validated PASS** (19/19 applicable rows; row 20 is Linux-only), Windows/Linux human rows pending (`editor/VALIDATION.md`). The Viewport is navigable: orbit / pan / dolly / fly + focus-selected. The `"No camera in scene"` overlay is gone (2.3.1 D19). **2.3.2 Selection & picking is CLOSED in code and macOS human-validated PASS** (21/21 applicable rows; row 22 is Linux-only), Windows/Linux human rows pending: click-pick via a local-space OBB ray test, `Ctrl`/`Cmd`/`Shift` modifiers, and an ImGui overlay wireframe box around the selection with the primary distinguished. One selection, three panels — the Viewport now writes into the same `Selection` the Hierarchy has always read and written. |
| **Next task** | **2.3.3 ImGuizmo transform gizmos** — see `docs/tasks/phase-2.md`. |
...
Test inventory at HEAD: **94** ctest entries with tools ON, **5** with `-DAERO_REFLECT_TOOLS=OFF -DAERO_SHADER_TOOLS=OFF` — unchanged by task 2.3.2 (its new coverage, `aero_editor_shell_test` 117 → 141 and `aero_editor_imgui_test` 14 → 19, rides existing targets/TUs, not new ctest entries — same shape 2.3.1 already established). Counts diverge by OS (Windows skips `golden-rule.include_scan_e2e`), so never assume one — measure with `ctest -N`.
