...
`aero_editor_shell_test` **1842 -> 1885**, `aero_editor_imgui_test` **181 -> 192**, the other five
...
**1404 / 1885 / 192 / 40 / 31 / 7 / 28** (`aero_tests`, `aero_editor_shell_test`,
...
`I140(c)` is a third subcase on an existing case and contributed ZERO**. **E.3.2 added FIFTEEN
`SUBCASE`s and they moved nothing; its three sabotage-forced commits added two whole cases (`EP12` and
one arm each on `RT2` and `EP2`) and moved the shell total by exactly one.** Never predict a delta
arithmetically.
...
merged — eleven commits, the full local gate green on both presets and both reduced configurations,
the 32-seed sabotage matrix run in full with three real holes found and closed, and NO existing GPU
case adjusted. Its validation page is UNRUN on all three platforms.** **E.3.1, E.3.3 and E.3.4 are
