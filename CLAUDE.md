...
Test inventory at HEAD: **94** ctest entries with tools ON, **5** with `-DAERO_REFLECT_TOOLS=OFF -DAERO_SHADER_TOOLS=OFF`, **18** with `-DAERO_REFLECT_TOOLS=OFF` alone — all three unchanged by every task since 2.4.1 (new coverage rides existing targets/TUs, not new ctest entries): `aero_tests` **363**, unchanged since 2.4.2; `aero_editor_shell_test` **169 → 199** (2.4.1) **→ 236 → 237 → 240 → 244** (2.4.2) **→ 259 → 264 → 274 → 288 → 289 → 296** (2.5.1) **→ 304** (2.5.2's `scene_golden_test.cpp` EG1–EG8) **→ 327 → 337 → 346 → 357 → 356 → 356** (2.6.1's nine commits: project.hpp/.cpp's 23 cases, +10 filesystem cases, +9 golden PG1–PG9, +11 flow-guard cases, −1 for the deleted `resolveProjectRoot` case, unchanged by the last two) **= 356 at HEAD**; `aero_scene_serialize_test` **12 → 22 → 23** (task 1.4.2's original twelve, then 2.5.2's G1–G4 and G5/G5b/G6/G7/G8/G9/G10 — **seven** physical `TEST_CASE`s in the "G5–G10" family, not six, because G5b is its own case; measure with `--list-test-cases`, never assume a plan's arithmetic); `aero_editor_inspector_test` **22**, unchanged since 2.4.2; `aero_editor_imgui_test` **41 → 46** (2.6.1's I18/I21–I24); `aero_editor_core` sources unchanged in link-line shape by 2.5.2 or 2.6.1 (2.6.1 added three new TUs — `project.cpp`, `project_file.cpp`, `project_ui.cpp` — plus a rename, `scene_file.cpp` → `text_file.cpp`, but **no new `target_link_libraries` entry**: the project format needs nothing from `aero::scene_serialize`). 2.4.1's review round fixed a blocking AC-16 defect and 2.4.2's two code-review rounds fixed a `placeAt`/`RootOrder::insert` replay-order defect; 2.5.1's code-review round fixed two blocking data-loss/UX findings (full history in `docs/10-engineering-log.md`'s 2.4.1/2.4.2/2.5.1 entries). 2.5.2 and 2.6.1 each ran a full sabotage matrix with no separate code-review round — both found and fixed their real bugs (2.5.2's EG6 log-drain inversion and out-of-bounds index; 2.6.1's unknown-key sweep order and `project.flow.requestedPath` leak) during implementation, before their commits. 2.5.2: all **19 of 19** seeds plus all **3** second-order checks, **six findings beyond the plan's own predictions** — most load-bearing: several writer-only seeds redden only the byte-comparing cases (G1–G4/G9/G10/editor EG3–EG5/EG7), never G5–G8; S12 (the mutual load+save bug, regenerated from a seeded build) proves the design is not theatre. 2.6.1: all **22 of 22** seeds plus all **3** second-order checks — **two seeds (S11, S15) proved completely undetected by the whole suite**, a real coverage gap rather than a construction-predicted non-discriminator: `createProject`'s `WriteFailed` rollback path and the `restoreLastProject == false` recents-skip are both unreachable/unobservable by any test in the tree; S9 (2.5.2's S12 re-proven for this format) confirmed the mutual-bug design but showed that regenerating only the nearest fixture can mask even a semantic case reading that same fixture (PG5), caught only by an unrelated fixture (PG6/PG7) or an unrelated multi-fixture loop (PG2) — full detail in `docs/10-engineering-log.md`'s 2.5.2/2.6.1 entries. `check-math-boundary.sh`'s scanned count: **209 → 215** (2.4.1) **→ 224** (2.4.2) **→ 232** (2.5.1) **→ 233 → 234** (2.5.2) **→ 235** (2.6.1 Step 1, `text_file.hpp`/`.cpp` rename) **→ 238** (Step 2, `project.hpp`/`.cpp`) **→ 239** (Step 3, `project_file.cpp`; unchanged through Steps 4–6) **→ 241** (Step 7, `project_ui.hpp`/`.cpp`) at HEAD. Counts diverge by OS (Windows skips `golden-rule.include_scan_e2e`), so never assume one — measure with `ctest -N`.
