...
| **Phase 2** — Editor | Epic 2.1 (ImGui integration) **CLOSED in code**, macOS human-validated PASS; Windows/Linux rows pending (`editor/VALIDATION.md`). Epic 2.2 (Core panels) **OPEN** — 2.2.1 Hierarchy, 2.2.2 Inspector, 2.2.3 Viewport and 2.2.4 Asset browser landed; 2.2.1–2.2.3 are each macOS human-validated PASS with Windows/Linux rows pending, 2.2.4's human pass is pending on all three OSes. |
