...
**Doctest, SEVEN binaries: 1088 / 1743 / 143 / 29 / 27 / 7 / 28**, against a measured baseline of
...
**`aero_editor_shell_test` +18** (`AD1`–`AD18`, one new TU) and **the other five +0**: this task adds
no ImGui case, no component, no serializer, no reflected field and no `reflect-gen` growth.
**The tools-OFF reading is the one that matters and it moves by the same +18 / +0**: measured in the
fresh `-G Ninja` reduced build, `AD*` lists **18** and the three `aero_tests` filters list the same
26 / 20 / 14 they do with tools on. `tests/editor/audio_decode_test.cpp` joins its target
...
3.6.3's **46** and 3.7.1's **78** move it not at all, and samples register no test. **Until 3.1.5 the triple moved only for `cooker.*` cases**
...
is the seven-binary line above (**1088 / 1743 / 143 / 29 / 27 / 7 / 28**), measured on this tree; the
