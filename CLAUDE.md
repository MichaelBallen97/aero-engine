...
alone — unchanged by task 3.1.1, which registers **zero** new ctest entries (every new case lives
inside an existing TU). `aero_tests` **389** (390's Phase-2-audit baseline was 363 before 3.1.1's
`guid_test.cpp`'s `GU1`–`GU26` — 26 new cases, all pinned-literal, no entropy source touched by any
test). `aero_editor_shell_test` **467** (Phase-2-audit baseline 390 → +77: `AM1`–`AM40` the `.meta`
format's naming/classification/parse/write battery, `AP1`–`AP18` the pure planner, `AG1`–`AG6` the
golden byte-fixpoint battery, `AD1`–`AD30` the real-disk `AssetDatabase` scan battery) — confirmed
**identical +77** in BOTH freshly-configured tools-OFF configurations (`build/tools-off-3.1.1` **443**,
`build/reflect-off-3.1.1` **443**, both containing all 77 `AM`/`AP`/`AG`/`AD` cases by
`--list-test-cases`, never assumed), which is AC-17's whole claim — the format and planner need no
serialization and are present, not skipped, in every reduced configuration. `aero_scene_serialize_test`
**23** and `aero_editor_inspector_test` **22**, both unchanged by 3.1.1. `aero_editor_imgui_test`
**51** (Phase-2-audit baseline 48 → +3: `I27`/`I28`/`I29`, the asset-scan-on-open, runtime-project-swap
and manual-rescan GPU-tier cases). `aero_editor_core` sources **41** (39 at the Phase 2 audit → +2:
