...
ten-row macOS pass is **5/10 PASS with every blank filled**; rows 2, 3, 5, 6 and 7 need the screen or Tracy and stay pending.**
...
checks with `headSha == HEAD` asserted before the merge, and macOS-validated **5/10 rows PASS** (2026-08-22) with **every measurement blank filled** — rows 1, 4, 8, 9 and 10; the default grid draws 670/1000 at yaw 0 and **1** at yaw 180 with a whole-run record of **0.183 ms against 0.714 ms** for `--no-cull` (−74.4 %), the 10648-instance grid **1.460 vs 4.115 ms** (−64.5 %), a degenerate projection draws all 1000 every frame and warns **exactly once** across 2066 frames, and the grid holds a vsync-paced 60.0 fps. **Rows 2, 3, 5, 6 and 7 stay PENDING — they need the screen or a Tracy session, and this machine grants neither Screen Recording nor Accessibility, so the window can be neither captured nor driven. No row failed.**.** The renderer learns to not draw. `.aeromesh` has carried `CookedBounds` per submesh and
...
`filters:` line, never from a `grep -c` of case names. 3.6.1 moves `aero_tests` **906 → 942** and
...
3.4.1's 57 new doctest cases, 3.4.2's 84, 3.5.1's 74, 3.5.2's 100, 3.1.5's **190** and 3.6.1's **36**
...
**Doctest, SEVEN binaries: 942 / 1725 / 138 / 29 / 27 / 7 / 28.** 3.6.1 adds **+36** to `aero_tests`
alone (`FC1`–`FC25` tier-0 and `CD1`–`CD11` behind `AERO_SHADER_TOOLS_ENABLED`, one new TU) and leaves
the other six **unmoved**; in a tools-off build the culling filter lists **25**, the CD tier absent by
