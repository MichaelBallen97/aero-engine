...
3.6.1 (Frustum culling) is COMPLETE IN CODE on `feat/3.6.1-frustum-culling` — eight green commits, the
full local gate green, the 30-seed matrix run to completion — but it is NOT pushed, NOT reviewed and
NOT merged, so it has no PR number and no merge commit yet, and its ten-row macOS pass has not run.**
...
**3.6.1 — Frustum culling: Epic 3.6 OPENS with it. COMPLETE IN CODE on
`feat/3.6.1-frustum-culling` (eight green commits), NOT pushed and NOT merged, and PENDING its ten-row
macOS pass.** The renderer learns to not draw. `.aeromesh` has carried `CookedBounds` per submesh and
...
Test inventory measured on `feat/3.6.1-frustum-culling` **(3.6.1 complete in code, on top of `main` @
`5a1bc69`)**, every number **re-measured on the tree in front of you, never derived by addition and
never carried forward from an earlier step or an earlier task** — read the totals from doctest's own
`filters:` line, never from a `grep -c` of case names. 3.6.1 moves `aero_tests` **906 → 940** and
nothing else; when it merges, re-measure rather than trusting that sentence.
...
3.4.1's 57 new doctest cases, 3.4.2's 84, 3.5.1's 74, 3.5.2's 100, 3.1.5's **190** and 3.6.1's **34**
move it not at all, and samples register no test. **Until 3.1.5 the triple moved only for `cooker.*` cases**
...
**Doctest, SEVEN binaries: 940 / 1725 / 138 / 29 / 27 / 7 / 28.** 3.6.1 adds **+34** to `aero_tests`
alone (`FC1`–`FC24` tier-0 and `CD1`–`CD10` behind `AERO_SHADER_TOOLS_ENABLED`, one new TU) and leaves
the other six **unmoved**; in a tools-off build the culling filter lists **24**, the CD tier absent by
its gate. The tracked list read **five** until 3.1.5, because
...
`editor`, 2 `engine/scene_render`, 7 `tests` and 1 `reflect-gen` fixture) and **404 → 408** at 3.6.1
(+4: `culling.hpp`, `culling.cpp`, `render_culling_test.cpp` and `samples/phase-3-culling/main.cpp` —
the sample's `CMakeLists.txt` and `README.md` are not C-family), with
`check-project-no-delete.sh`'s Check B scan **66 → 73** at 3.1.5 and **unmoved at 73** through 3.6.1,
which adds no `editor/src/*.cpp`. All re-measured **after `git add`**, since `git ls-files` sees
...
records, 37 keys, the format's single padding site and 47 values. **3.6.1 commits NO artifact and NO fixture of any kind** — `samples/phase-3-culling` draws only the
renderer's three built-in primitives, whose boxes are folded at `create()` from the geometry itself, so
there is nothing on disk to keep in step. **It adds NO new test fixture
