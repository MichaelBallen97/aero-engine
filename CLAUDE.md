...
ten-row macOS pass is ✅ **PASS on ALL 10 ROWS** with every blank filled. 3.6.3 (Tonemap/gamma pass)
is COMPLETE IN CODE on `feat/3.6.3-tonemap-gamma-pass` (nine commits, cut from `main` @ `64df342`) —
the full local gate green, the 31-seed matrix run to completion with three genuine gaps all closed
structurally and re-proven, and its twelve-row macOS pass PENDING. It is NOT merged and no CI run
exists for it yet. 3.6.2 (Directional shadow map) is being built in parallel off the same branch
point, in a separate worktree.**
...
**3.6.3 — Tonemap/gamma pass: COMPLETE IN CODE on `feat/3.6.3-tonemap-gamma-pass`, nine commits, cut
...
Test inventory measured on `feat/3.6.3-tonemap-gamma-pass` **(3.6.3 complete in code, cut from `main`
@ `64df342`, i.e. on top of merged 3.6.1)**, every number **re-measured on the tree in front of you,
never derived by addition and never carried forward from an earlier step or an earlier task** — read
the totals from doctest's own `filters:` line, never from a `grep -c` of case names. 3.6.3 moves
`aero_tests` **942 → 984** and `aero_editor_imgui_test` **138 → 142**, and nothing else; when it
merges, re-measure rather than trusting that sentence. **3.6.2 is being built in parallel off the same
branch point, so whichever lands second re-measures everything.**
...
**Doctest, SEVEN binaries: 984 / 1725 / 142 / 29 / 27 / 7 / 28.** 3.6.3 adds **+42** to `aero_tests`
(`TM1`–`TM29` tier-0 and `PP1`–`PP13` behind `AERO_SHADER_TOOLS_ENABLED`, one new TU) and **+4** to
`aero_editor_imgui_test` (`I103`–`I106`), leaving the other five **unmoved**; in a tools-off build the
tonemap filter lists **29**, the PP tier absent by its gate, and `aero_editor_imgui_test` reads **128**
rather than 124, because **all four** editor cases run in that configuration and `I105`'s tools-OFF
arm is the only place AC-16's degradation path is tested at all. **That last number is a real check,
not a decoration**: the four cases first landed inside 3.1.5's file-level
`#if AERO_SHADER_TOOLS_ENABLED` block, everything was green, and `I105`'s tools-OFF arm never ran —
**counting cases in the reduced configuration is what caught it, because a suite that omits a case
passes just as loudly as one that runs it**.
Before 3.6.3 the totals read **942 / 1725 / 138 / 29 / 27 / 7 / 28**; 3.6.1 had added **+36** to
`aero_tests` alone (`FC1`–`FC25` tier-0 and `CD1`–`CD11` GPU-gated), with the culling filter listing
**25** in a tools-off build. The tracked list read **five** until 3.1.5, because
...
the sample's `CMakeLists.txt` and `README.md` are not C-family) and **408 → 415** at 3.6.3 (+7:
`tonemap.hpp`, `tonemap.cpp`, `post_process.hpp`, `post_process.cpp`, `tonemap_pack.hpp`,
`render_tonemap_test.cpp` and `samples/phase-3-tonemap/main.cpp` — the two HLSL files are not
C-family either), with
`check-project-no-delete.sh`'s Check B scan **66 → 73** at 3.1.5 and **unmoved at 73** through 3.6.1
and 3.6.3, neither of which adds an `editor/src/*.cpp`. All re-measured **after `git add`**, since `git ls-files` sees
