...
- **The editor's human-validation records are LOCAL-ONLY and untracked** (2026-07-31): `editor/VALIDATION.md` and everything under `editor/validation/` are gitignored, alongside `docs/plans/` and `docs/specs/`. They still exist on the maintainer's machine and are still the place a human pass is recorded — they are simply not on the remote, so **a fresh clone will not have them** and every reference to them in tracked files (this file, `docs/10-engineering-log.md`, `.claude/rules/editor.md`, and comments in a few editor sources and tests) points at something only the maintainer can see. `samples/phase-0-cube/VALIDATION.md` and `samples/phase-1-scene/VALIDATION.md` are **still tracked** — the Phase 0 and Phase 1 gate records stay on the remote. When a validation status changes, update the local page **and** the summary in this file's state block, because the state block is now the only tracked record of it.
