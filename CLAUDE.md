...
Test inventory at HEAD: **94** ctest entries with tools ON, **5** with `-DAERO_REFLECT_TOOLS=OFF -DAERO_SHADER_TOOLS=OFF`, **18** with `-DAERO_REFLECT_TOOLS=OFF` alone — all three unchanged by task 2.4.1 (its new coverage rides existing targets/TUs, not new ctest entries): `aero_editor_shell_test` **169 → 196** (`command_stack_test.cpp` C1–C18, `transform_command_test.cpp` T1–T9); `aero_editor_imgui_test` **24 → 30** (I1–I6); `aero_tests` unchanged at **356**; `aero_editor_core` **25 → 27** sources. Counts diverge by OS (Windows skips `golden-rule.include_scan_e2e`), so never assume one — measure with `ctest -N`.
