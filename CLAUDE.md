...
| **Phase 2** — Editor | Epic 2.1 (ImGui integration) **CLOSED in code**, macOS human-validated PASS; Windows/Linux rows pending (`editor/VALIDATION.md`). Epic 2.2 (Core panels) **CLOSED in code** — 2.2.1 Hierarchy, 2.2.2 Inspector, 2.2.3 Viewport, 2.2.4 Asset browser and 2.2.5 Log/console panel all landed; **no `PlaceholderPanel` remains**. 2.2.1–2.2.4 are macOS human-validated PASS with Windows/Linux rows pending; **2.2.5 is macOS human-validated PARTIAL — 11 of 15 rows PASS, 4 BLOCKED because the editor emits no log record after startup**, so they cannot be run at all. The gate stays open pending a triggerable runtime log source and a non-macOS pass (`editor/VALIDATION.md`) — two triggerable sources now exist: 2.3.3's degenerate/non-finite-transform gizmo WARN (select any single entity, zero one of its own `Transform.scale` components in the Inspector, then drag its own gizmo handle) and, as of 2.4.1, a Debug-build `⌘Z` — one `DEBUG editor: undo 'Transform' (…)` record per undo — though the 2.2.5 rows themselves were not re-run either time. **Epic 2.3 (Manipulation) is CLOSED in code** — 2.3.1 Editor camera, 2.3.2 Selection & picking and 2.3.3 ImGuizmo transform gizmos all landed. All three are macOS human-validated PASS — 2.3.1 19/19, 2.3.2 21/21 and 2.3.3 **24/24** applicable rows (2.3.3's row 23 is Linux-only) — with Windows/Linux rows pending for all three (`editor/VALIDATION.md`). The Viewport is now the whole edit → select → move loop: orbit/pan/dolly/fly navigation (2.3.1), click-to-select with a wireframe highlight (2.3.2), and translate/rotate/scale gizmos in local or world space with hold-to-snap (2.3.3), writing through a new `transform_ops` seam. **Epic 2.4 (Undo/redo) is OPENED — 2.4.1 (Command stack) landed in code**: a `Command`/`CommandStack` backbone with an explicit merge chain and a 128-entry bounded history; `Ctrl+Z`/`Ctrl+Shift+Z` with key repeat and live Edit-menu items (`"Undo <label>"`/`"Redo <label>"`); and `TransformCommand` wrapping `transform_ops`, with the Viewport gizmo routed through it so one continuous drag is one undo step. This **completes Epic 2.3's Definition of Done's *undoably* clause one task earlier than the docs previously said** (`docs/tasks/phase-2.md` now says so explicitly) — 2.4.2 still owns the Inspector and the structural (create/delete/reparent) commands. **macOS human-validated PASS — 16/16 rows (2026-07-30)**, which is what confirms the code-review round's AC-16 ordering fix (one continuous drag = one undo step) end to end; Windows/Linux rows pending. That pass also re-confirms a **second triggerable log source** for 2.2.5's four BLOCKED rows: a Debug-build `⌘Z` logs one `DEBUG editor: undo 'Transform' (…)` per undo (silent in Release), verified by row 14 — so **nothing blocks those four rows any more**, they simply have not been re-run. |
| **Next task** | **2.4.2 Property-set + structural commands** — see `docs/tasks/phase-2.md`. |
...
Test inventory at HEAD: **94** ctest entries with tools ON, **5** with `-DAERO_REFLECT_TOOLS=OFF -DAERO_SHADER_TOOLS=OFF`, **18** with `-DAERO_REFLECT_TOOLS=OFF` alone — all three unchanged by task 2.4.1, including its code-review round (new coverage rides existing targets/TUs, not new ctest entries): `aero_editor_shell_test` **169 → 196 → 199** (`command_stack_test.cpp` C1–C18 plus the review round's C19–C21, `transform_command_test.cpp` T1–T9); `aero_editor_imgui_test` **24 → 30** (I1–I6); `aero_tests` unchanged at **356**; `aero_editor_core` **25 → 27** sources. The review round fixed a blocking AC-16 defect (a release-frame gizmo drag recorded as two history entries, not one — see `docs/10-engineering-log.md`'s 2.4.1 code-review-round entry) and corrected four sabotage-verdict rows in `editor/VALIDATION.md` that the first pass had gotten wrong. Counts diverge by OS (Windows skips `golden-rule.include_scan_e2e`), so never assume one — measure with `ctest -N`.
