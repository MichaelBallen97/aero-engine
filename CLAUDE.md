...
**EPIC E.3 (Inspector & context routing) IS CLOSED IN CODE — all four tasks merged.** E.3.1
(axis-labelled vector fields), E.3.2 (selection-follows-focus router) and E.3.3 (asset-reference
picker) are merged and macOS-validated; **E.3.4 (material inspector redesign) CLOSES THE EPIC — eight
commits, the full local gate green on both presets and both reduced configurations, the 54-seed
sabotage matrix run in full and a code-review round closed. Its validation page is written and has NOT
been run on any platform.** E.1.5's page and E.3.2's are the other two unrun. **THE BUILT-IN COUNT
STAYS TEN: E.3.4 adds no component, so the five-generation-site rule and the component-count sweep do
not fire for it at all.** E.3.2 landed before E.3.1, which was legal and disjointly id-reserved; that
reservation is discharged and the numbering is contiguous.
...
configurations. **The other 11 tasks are planning only.** Six epics, 24
...
### E.3.4 — Material inspector redesign (MERGED) — CLOSES Epic E.3, and a chrome that did not fit

**The Material panel reads as a material now.** A fixed identity line and live preview whose height is
DERIVED from the panel's; a scrolling body of eight `CollapsingHeader` sections, five pairing a texture
slot with the scalars that scale it under ONE shared label column; a fixed footer with Apply, Revert,
the dirty word and one always-reserved status line. Each slot is **one ImGui item** — the E.3.3 picker's
button grown tall enough to hold the bound asset's thumbnail inside its own frame, `Clear` beside it,
the colour-space note beneath — and the six sampler tokens sit behind a per-slot disclosure that starts
closed and is drivable from a seam. One new public, pure, ImGui-free pair (`material_inspector_model`)
owns every number, label, sentence and priority order the panel decides. **Eight commits. NO component,
NO shader, NO new target, NO new ctest entry, NO `docs/09` change, NO link-line change.** `ctest -N`
**178 → 178** in both presets and **165 / 93** reduced; doctest **1404 / 1936 / 215 / 40 / 59 / 10 /
28**; guards **519 / 92 / 163 / 92 / 165 / A=6 B=87 / 11-3-55 / 6-57**, math and prong B the only two
that move. **Its validation page is written and UNRUN on every platform.** Full detail in `docs/10`; the
sentences that govern new work are below.

**1. ★ ImGui's `StateStorage` IS PER WINDOW, SO A BODY THAT IS SOMETIMES A CHILD IS TWO SETS OF WIDGET
STATE.** `TreeNodeSetOpen` writes through `g.CurrentWindow->DC.StateStorage`
(`imgui_widgets.cpp:6802`) and `DC.StateStorage = &window->StateStorage` (`imgui.cpp:8581`). A child is
a distinct window, so the same `CollapsingHeader` is a different entry inside the body child than in the
panel window — **collapsed sections silently re-opened from `DefaultOpen` whenever the panel crossed
the mode boundary**, a regression against `main`, which had no child at all. The eight sections carry
**panel-owned** open state now, exactly as the sampler disclosure already did. **One function called
from two places keeps the two modes drawing the same CODE; that is not the same as the same STATE.
Anything ImGui keys by window must be a panel member to survive a mode flip.**

**2. ★ A SEAM'S OWN ACCESSOR IS A ROUND TRIP, NOT A TEST.** Reading back the flag a seam just wrote
reports what was *requested*, never whether ImGui *obeyed*. Sabotage seed `S26` (`ImGuiCond_Once`
instead of `Always`, which hands the node back to ImGui's storage so it never opens) walked through
**149 green assertions** for that reason, with five `BeginCombo` calls and a `DragInt` executing on no
lane at all. **Assert a consequence the widget PRODUCED** — `samplerRowsDrawn()` counts disclosures that
actually submitted their rows. The same shape is what makes the section regression assertable.

**3. ★ THE STYLE IS DOUBLED ON A RETINA DISPLAY AND THE FONT IS NOT, WHICH IS WHY THE FIRST DESIGN DID
NOT FIT.** Measured before any code was written, in the 320x180 window five GPU cases drive:
`availHeight` **98** (and 418 at 800x500, so `availHeight == windowHeight − 82`), `fontSize` 13,
`frameHeight` **25**, `textLineHeight` 13, `itemSpacingY` **8**, `SeparatorSize` **2** — display scale
**2.0**, because `ScaleAllSizes(SDL_GetWindowDisplayScale(win))` runs unconditionally at
`imgui_layer.cpp:87-89` while `io.ConfigDpiScaleFonts` only overwrites `FontScaleDpi` when a monitor's
DPI **CHANGES**, which never fires for a window created already on the 2x display. So the fixed chrome
is **103 points against 98**, not the 71 the unscaled defaults give. A layout with one mode answers that
with a **zero-height preview** — failing `I99`, `I135` and AC-3 **on every Retina Mac while the three 1x
CI lanes stay green**. `MaterialPanelLayout` therefore carries a MODE, the preview is floored in both,
and the threshold is the one value that makes the height **continuous** across the boundary. **That DPI
gap is E.6.1's, not this task's.**

**4. `footerHeight` NEEDS THREE `ItemSpacing` TERMS, AND `Separator()`'s HEIGHT MUST BE READ.**
`EndChild` calls `ItemSize(child_size)` (`imgui.cpp:6860`) and `ItemSize` advances by
`line_height + ItemSpacing.y` (`:12129`), so the child's bottom edge is followed by a spacing BEFORE the
separator; `imgui.cpp:437` spells the recipe. Measured **45 points at 1x, 64 at 2x**. And **1.92.8
removed the "a 1 px `Separator` does not move the cursor" hack while `SeparatorEx`'s own header comment
at `:1657` still describes it** — the code is commented out at `:1703-1708`, and `ScaleAllSizes` scales
the value. Read the live style; `I172(c)` pins that the panel states no bare separator magnitude.

**5. `S47` IS COVERED, AND THE PLAN'S CLAIM ABOUT ASan WAS WRONG.** Inlining a by-value
`materialSlotFieldKey` into an aggregate whose `fieldKey` is a `string_view` does **not** survive
silently: ASan reports **`stack-use-after-scope`**, a read of size 6 at offset 192 in the caller's own
frame, and `I164` aborts. That is a **different check** from `detect_stack_use_after_return` — the
latter is off by default and covers a *returned* frame; the former is **on** and covers a temporary that
died in the *current* one. `keyScratch` stays for the same reason it always should have.

**6. `I136` IS RED AT `HEAD` ON A 2x DISPLAY AND IS NOT E.3.4's.** It fails
`REQUIRE(drawExtent.width > 4U)` with value **4**, deterministically, on an unmodified tree — the same
2x cause on the **width** axis, since at 320 points wide the Right dock node is effectively collapsed
and `I136` ticks three times without a `requestLayoutReset()`. **The local GPU tier gates at 213/214
with it named**, never quietly counted as green. **Handed to E.6.1** with the DPI mechanism.

### E.3.3 — Asset-reference picker (MERGED) — a reference is something you PICK now

**You can set a `mesh`, a `material` and a `clip` from the Inspector, and the field says what it wants.**
`AERO_ASSET(kind)` is the **fourth** field annotation; a **fifth `DropSurface`** with a NON-DEFAULTED
fourth parameter routes the field through 3.1.5's own matrix; the thumbnail ledger and store live in one
**`ThumbnailService`** with three consumers and one budget. Sixteen commits, merged as PR #104
(`fc77c4b`), **macOS-validated 10 PASS / 3 PARTIAL / 1 NOT EXECUTABLE / 1 NOT RUN, nothing failed.**
Four sentences still govern new work: **the VOCABULARY SPLIT** — reflect-gen validates a *grammar* and
the **editor** owns the *vocabulary*, because `AssetKind` is an editor type the engine's reflect layer
must not know, so **do not add a kind list to the tool**; **`ImGuiListClipper::IncludeItemByIndex` goes
AFTER `Begin()`**, because the constructor `memset`s `DisplayStart` to 0 and `Begin` is what sets it to
−1, so a call above it is an `IM_ASSERT` abort (`imgui.cpp:3418`); **an API-positioned popup is never
clamped** (`imgui.cpp:8279`) and one off the screen has its child **culled**, which makes
`ImGuiListClipper::Step()` return false immediately — so `assetPickerAnchor` clamps both axes itself,
and any future API-positioned window inherits it; and **a label is truncated at its first `##`**
(`imgui.cpp:3918`) while **a `Selectable`'s item box is not its tile** (`imgui_widgets.cpp:7395-7396`),
neither of which has automated cover anywhere.
...
* **`/editor`** gained ONE public pair at E.3.4 (`material_inspector_model` -- PURE: no ImGui, no GPU, no `<filesystem>`, no logging, so tier 0 can walk the whole panel's shape), on top of E.3.3's four (`thumbnail_service`, `asset_tile`, `asset_picker_model`, `asset_picker`), E.3.2's two (`context_router`, `editor_prefs`), E.2.4's one (`material_preview_rig`) and E.2.3's two (`viewport_icons`, `viewport_gizmos`). **THE "PAIR COUNT" THIS LINE USED TO CARRY IS DROPPED, BECAUSE IT IS NOT REPRODUCIBLE**: E.2.4 measured the tree six ways looking for the 28/30 figure recorded here since E.1.5 and **none of the six is it**. The two figures anyone can re-run are `git ls-files`: **`editor/src/*.cpp` 86 -> 87** and **`editor/include/aero/editor/*.hpp` 61 -> 62** at E.3.4. Use those; `/tools` links `aero::assets` and `aero::editor_core`
...
**`ctest -N` 178 -> 178**, entry set byte-identical, and **165 / 93** in the two reduced configurations,
each configured FRESH. doctest **1404 / 1936 / 215 / 40 / 59 / 10 / 28**, up from E.3.3's
**1404 / 1915 / 209 / 40 / 59 / 10 / 28**. **EXACTLY TWO of the seven move and FIVE must not**: shell
1915 -> 1936 by the `MR1`–`MR21` battery and imgui 209 -> 215 by `I170`–`I175`, while `aero_tests`
(1404) says no engine behaviour changed, `aero_scene_serialize_test` (40) and `aero_editor_inspector_test`
(59) say no serialized change and no Inspector change crept in — E.3.4 edits `inspector_panel.cpp` and
those 59 cases pass **unedited**, which is the whole proof that the width refactor was behaviour-free —
and `aero_reflect_meta_test` (10) and `aero_reflect_json_test` (28) say no reflection change did. **A
moved one of those five is a stop-and-find-out.** Guards
**519 / 92 / 163 / 92 / 165 / A=6 B=87 / 11-3-55 / 6-57**: math and prong B move by E.3.4's **three** new
tracked C-family files of which **one** is an `editor/src/*.cpp` — so prong B moves by ONE, not three —
and the other six are unmoved from E.3.3's **516 / 92 / 163 / 92 / 165 / A=6 B=86 / 11-3-55 / 6-57**.
**A THIRD guard moving is a stop-and-find-out.**

**AND THE LOCAL GPU TIER GATES AT 213 OF 214, NOT 214 OF 214.** `I136` fails
`REQUIRE(drawExtent.width > 4U)` with value **4** on a 2x display, deterministically, **on an unmodified
`HEAD`** — it is pre-existing, it is the DPI story's, and it is handed to E.6.1. Name it; never let a
known failure be quietly counted as green, and never let it hide a new one.
...
The eight guard counts at **E.3.4's** gate: math **519**, platform **92**, rhi **163**, scene **92**,
golden-rule **165**, project-no-delete **A=6/B=87**, audio **11/3/55**, probes **6/57** — math and
project-no-delete moved by E.3.4's three new tracked C-family files, of which exactly one is an
`editor/src/*.cpp`, and the other six are unmoved from E.3.3's
**516 / 92 / 163 / 92 / 165 / A=6 B=86 / 11-3-55 / 6-57**. **A THIRD guard moving on a task of this
shape is a stop-and-find-out.** **The two
...
under a CONTINUOUS divider drag is judged — the extent now moves with the panel's HEIGHT as well as its
width, bounded by `nextTargetExtent`'s quantisation and its hysteretic shrink arm), **row 3** (the slot
row being one item: the thumbnail's four gaps inside the button frame, no caption, the sentence not
overlapping, `Clear`'s position — `S32`/`S33`/`S34`/`S36`), **row 9** (the header not moving when a
notice appears, `S40`/`S41`/`S42`) and **row 1** (one label column across all eight sections, `S36`/`S44`).
**`S47` is NO LONGER on that list** — it aborts under ASan, so it has automatic cover on all three Debug
lanes and validation row 3 is no longer its only witness.

E.3.3's page IS macOS-run — 10 PASS / 3 PARTIAL / 1 NOT EXECUTABLE / 1 NOT RUN, 2026-09-21, nothing
failed, 43 of 51 records ticked. E.3.2's is twelve rows
...
**Phase E is the open front, and EPICS E.1, E.2 AND E.3 ARE ALL CLOSED IN CODE — thirteen of 24 tasks
merged. E.4, E.5 and E.6 are what is left: eleven tasks, planning only.**

**E.3.4 (material inspector redesign) is the newest and it CLOSES Epic E.3** — eight commits, the full
local gate green on both presets and both reduced configurations, the 54-seed sabotage matrix run in
full and a code-review round closed. It took the ImGui-tier case-id ceiling to **`I175`**; `I170`–`I175`
are claimed, so **`I176` is the next free id**. The `MR` prefix is now taken by its tier-0 battery
(`MR1`–`MR21`). **Its validation page is written and UNRUN.**

**Two seams it added, both readable from `EditorApp` and both the preview accessors' posture**:
`requestMaterialSlotDetails`/`materialSlotDetailsOpen` (the sampler disclosure) and
`requestMaterialSectionOpen`/`materialSectionOpen` (the eight sections), plus one observable,
`materialSamplerRowsDrawn()`. **The observable is the point**: a seam's own accessor reads back what was
REQUESTED, so it can never say whether ImGui obeyed — which is how seed `S26` walked through 149 green
assertions. Any future panel state driven by a seam inherits that, and the answer is to assert a
consequence the widget produced.

**Three handoffs discharged.** E.3.3's *"a thumbnail in the row (E.3.4's)"* — **taken**, and in the form
E.3.3's own D3 implied: the widget grew by one appended defaulted member rather than the row growing by
an item. The Material panel's *"five identical dropdowns"* clause of the roadmap goal — already gone at
E.3.3 and now genuinely a picker with a thumbnail, a clear action and a colour-space note around it.
And `assetReferenceFieldWidth` is now ONE formula with TWO hosts, so a theme or DPI change moves the
Inspector's reference row and the material slot's together.

**Two NEW handoffs, both to E.6.1.** The **DPI story** gains a measurement it did not have: the editor
calls `ScaleAllSizes(SDL_GetWindowDisplayScale(win))` unconditionally while `io.ConfigDpiScaleFonts` only
overwrites `FontScaleDpi` when a monitor's DPI **changes** — which never fires for a window created
already on a 2x display — so the style is doubled and the font is not. And **`I136`**, red at `HEAD` on a
2x display for the same reason on the width axis, is E.6.1's to fix; **the local GPU tier gates at
213/214 with it named.**

**E.3 closes with exactly TWO Inspector-row gaps still open** — E.2.1's enum-aware row and E.2.2's
unit-aware row, both reflect-gen surface first — **and E.3.4 adds none and closes none.** It adds one
gap of its own shape, unowned: a **closed `File` section**, which needs one flag plus nothing else now
that `requestMaterialSectionOpen` exists, the day a manual pass says the diagnostics are noise.
...
**E.4.5 also inherits a THIRD piece of evidence from E.3.4**: the material slot's row is a second,
row-sized consumer of `drawAssetTileFace`, which is what says the face's geometry parameters generalise
past a grid tile. **E.3 CLOSED with exactly TWO Inspector-row gaps still open** — E.2.1's enum-aware row
...
E.2.4, E.3.2 and E.3.4 all add none.** **E.3.1** still inherits the palette key; E.2.3's four gizmo tints join
