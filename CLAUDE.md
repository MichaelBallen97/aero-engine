...
- **There are no build/test/lint commands yet.** When the build lands (Phase 0) it will be CMake + `CMakePresets.json` + vcpkg (manifest mode, pinned baseline), tests with doctest, `clang-format`/`clang-tidy` enforced in CI, and a `tools/reflect-gen` codegen step that runs before compilation — all specified in `docs/04-conventions-setup.md`. **Update this file with the real commands once they exist.**
