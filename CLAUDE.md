...
**Planning is complete; Phase 0 (Foundations & First Triangle) is in progress — no *compiled* code exists yet, but the build skeleton does.** This is a git repository with a CMake + pinned-vcpkg build, six host-gated presets, a 3-OS GitHub Actions CI matrix, and the canonical folder layout; there are still zero compiled targets (real targets start at 0.2.1). Epic 0.1 tasks landed so far: 0.1.1 (repo + CMake skeleton), 0.1.2 (vcpkg manifest), 0.1.3 (CI matrix). Since the July 2026 re-plan, **every phase (0–8) is broken down to epic → task → subtask depth** in `docs/tasks/phase-N.md` — `docs/07-tasks.md` is the index. Numbering is append-only (new work takes the next free number; existing numbers never change).
