...
- **Build commands (as of 0.1.1 — skeleton only, zero targets yet):** configure with `cmake --preset macos-debug` or `cmake --preset macos-release` (Windows/Linux presets exist but are gated to their host OS by preset conditions), then build with `cmake --build --preset macos-debug` / `cmake --build --preset macos-release`. There is nothing to compile yet — `add_subdirectory()` wiring exists, real targets start at task 0.2.1. vcpkg (0.1.2), doctest (0.1.4), and `tools/reflect-gen` (Phase 1) are not wired in yet — all specified in `docs/04-conventions-setup.md`. **Update this note again as each of those lands.**
