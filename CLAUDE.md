...
configurations. **The other 14 tasks are planning only.** Six epics, 24
...
### E.3.3 — Asset-reference picker (MERGED) — a reference is something you PICK now

**You can set a `mesh`, a `material` and a `clip` from the Inspector, and the field says what it
wants.** `AERO_ASSET(kind)` is the **fourth** field annotation, mirrored by `FieldUiMeta::assetKind`
(a `const char*`, appended last, sparse); a **fifth `DropSurface`** (`AssetField`) with a
**NON-DEFAULTED** fourth parameter routes the field through 3.1.5's own matrix; the thumbnail ledger
and store moved into one **`ThumbnailService`** with three consumers and one budget; and one
src-private widget, `drawAssetReferenceField`, serves both the Inspector's `Guid` rows and the Material
panel's five slots. **SIXTEEN commits.** `ctest -N` **174 -> 178** — its FIRST move since 3.7.3, and
the reason is four `reflect-gen.asset_*` process cases, not a target — with the entry SET otherwise
byte-identical and **zero removals**; **165** shader-tools-OFF, **93** reflect-tools-OFF. doctest
**1404 / 1915 / 209 / 40 / 59 / 10 / 28**; guards **516 / 92 / 163 / 92 / 165 / A=6 B=86 / 11-3-55 /
6-57** (math and Check B are the only two that move, and a third moving is a stop-and-find-out);
`git ls-files` **86 / 61**. **THE BUILT-IN COUNT STAYS TEN** — two component headers gain annotations
and nothing else, both `sizeof` pins hold, and neither the five-generation-site rule nor the
component-count sweep fires. **NO `docs/09` change, NO shader, NO new dependency, NO link-line change**;
the whole `engine/` diff is THREE files. **Its validation page is written and UNRUN on every platform.**
Full detail in `docs/10`; the sentences that govern new work are below.

**1. THE VOCABULARY SPLIT.** reflect-gen validates a **grammar** — the token is an identifier — and
passes it through; the **editor** owns the **vocabulary**, because the set of kinds is `AssetKind`, an
editor type the engine's reflect layer must not know. `shader` PARSES and is emitted verbatim; the
editor refuses it as *unconstrained plus one warning*, never an error. **Do not add a kind list to the
tool.** The reflectable subset does NOT grow: this is a UI hint on an existing category, `--emit-json`
is untouched, and `annotations_meta`/`guid_meta` staying green **unedited** is what proves an
unannotated field's generated bytes did not move.

**2. `ImGuiListClipper::IncludeItemByIndex` GOES AFTER `Begin()`, NEVER BEFORE IT — and the plan said
the opposite.** The constructor `memset`s `DisplayStart` to **0** and `Begin` is what sets it to −1, so
a call above `Begin` trips `IM_ASSERT(DisplayStart < 0)` (`imgui.cpp:3418`, whose own comment says
*"Only allowed after Begin()"*) — an **abort** in the Debug build — and dereferences a null `TempData`.
The header's *"before the first call to `Step()`"* reads as if above `Begin` were safe.

**3. AN API-POSITIONED POPUP IS NEVER CLAMPED, AND ONE OFF THE SCREEN HAS ITS CHILD CULLED — WHICH
SUBMITS NOTHING AND SAYS NOTHING.** `Begin`'s visibility clamp runs only `if (!window_pos_set_by_api &&
…)` (`imgui.cpp:8279`). A popup taller than the work area has room in neither direction, stays below its
button, and was placed past the bottom of the screen, where ImGui culls the grid child — and a culled
child makes `ImGuiListClipper::Step()` return **false immediately**. **Measured in a 320x180 window: the
grid drew on the popup's appearing frame and on no frame after it**, so the shared service saw an empty
visible set every tick and its eviction protection could never apply. `assetPickerAnchor` clamps BOTH
axes now, applied to the window's **effective top** so the `(0,1)` flip pivot is honoured rather than
clamped as if it were a top-left. **Any future API-positioned window inherits both halves.**

**4. A COUNT IS NOT A CHOICE, AND A COMPOSITION NOTHING REACHES IS NOT A COMPOSITION.** `I162`'s
`material` arm asserted `candidateCount == 1` — and the fixture holds exactly one Model and exactly one
Material, so a `material` field wrongly annotated `model` still lists **one**. It commits and reads the
guid back now. Separately, `Folder`/`Text`/`Unknown` shared a matrix arm that returned `None`
**directly**, so the `assetKindIsDraggable` term inside the composed row was **dead** — and the property
the composition exists for (widen the predicate, and the drag source, the token vocabulary and this
surface widen together) was quietly false. They reach the row now, in **one** arm with `Audio`, because
draggability is the only thing that separates them and it is decided inside the row.

**5. A LIVE ONE-SHOT THAT MERELY WAITS FOR A POPUP IS A DELAY LINE.** A `pendingCommit` issued while
nothing is open must be **DROPPED**, not held: held, it fires on the very frame the next popup appears
and closes it again. **"The undo count did not move" cannot see that**, because committing the bound
asset is refused for equality anyway — the discriminator is that the next open **stays open**. And the
observables are **owner-keyed**, because `MeshRenderer` draws TWO `Guid` rows every frame and a per-field
write has the second clobber the first — **which is exactly why a tick in which NO field claimed the
popup must reset them from OUTSIDE the widget.** The owning row stops being drawn the moment the
selection moves to an entity without that component; ImGui closes the popup, and nothing inside a widget
that is not running can say so. The widget stamps and `tick()`'s post-draw slot consumes, because only a
slot that runs after the WHOLE draw walk can know nobody claimed it. **`MeshRenderer` also hides this**:
its second `Guid` row performs the "nothing is open" clear on the first row's behalf one tick later, so
an arm written over it decides nothing — `I169` drives a single-`Guid`-field component for that reason.

**6. A GREP FOR `error:` IS NOT A BUILD VERDICT, AND THE FULL-BRANCH LINT IS NOT THE UNION OF THE
PER-COMMIT ONES.** The fifth `DropAction` made two switches in `editor_app.cpp` non-exhaustive — the
`-Wswitch` signal the `default:`-free design exists to produce — and it survived six commits, because
each step grepped for `error:` and every later build was incremental. The **forced-recompile sweep over
the whole branch diff** is what found it. Likewise `bugprone-branch-clone` appeared in a file a later
commit did not touch last.

**7. A LABEL IS TRUNCATED AT ITS FIRST `##`, AND A `Selectable`'s ITEM BOX IS NOT ITS TILE.** Two
draw-list rules, both from the code-review round, and **neither has automated cover anywhere** — nothing
in this tree reads rendered text or measures a widget rect, and both seeds came back green. (a) The
value sentence is drawn with `ImDrawList::AddText`, clipped to the button's frame, and the button's
label is the `###` id ALONE: `FindRenderedTextEnd` stops at the first `##` (`imgui.cpp:3918`), so
`readme##v2.png` would render as `readme` and `##notes.png` would leave the button blank. An empty
visible label still measures one line high (`imgui.cpp:6447-6448`), so the frame height is unchanged,
and `AddText` submits no item, so the drop target and the anchor are unmoved. The Asset Browser's tile
caption has documented this since 3.1.3 (E19). (b) A tile's origin is `GetCursorScreenPos()` taken
**before** the `Selectable`, never `GetItemRectMin()` after it: a `Selectable` extends its item box on
the MIN side by half the item spacing (`imgui_widgets.cpp:7395-7396`).
...
* **`/editor`** gained FOUR source pairs at E.3.3 (`thumbnail_service`, `asset_tile`, `asset_picker_model`, `asset_picker` -- only `asset_picker_model.hpp` is public; the other three headers are src-private because each names ImGui, the GPU or both) on top of E.3.2's two (`context_router`, `editor_prefs`), E.2.4's one (`material_preview_rig`) and E.2.3's two (`viewport_icons`, `viewport_gizmos`). **THE "PAIR COUNT" THIS LINE USED TO CARRY IS DROPPED, BECAUSE IT IS NOT REPRODUCIBLE**: E.2.4 measured the tree six ways looking for the 28/30 figure recorded here since E.1.5 and **none of the six is it** — the metric behind that number is undefined, and it had already been flagged as carried arithmetic rather than a measurement. The two figures anyone can re-run are `git ls-files`: **`editor/src/*.cpp` 82 -> 86** and **`editor/include/aero/editor/*.hpp` 60 -> 61** at E.3.3, which added FOUR src pairs (`thumbnail_service`, `asset_tile`, `asset_picker_model`, `asset_picker`) of which one header is public. Use those; `/tools` links `aero::assets` and `aero::editor_core`
...
At **E.3.3's** gate — measured on both presets, out of freshly built trees, and agreeing between them:
**`ctest -N` 174 -> 178**, its FIRST move since 3.7.3, with the entry **SET** differing by exactly the
four `reflect-gen.asset_*` names and **ZERO removals** (compared by name against a configured worktree
at the branch point, not carried). doctest **1404 / 1915 / 209 / 40 / 59 / 10 / 28**, up from E.3.1's
**1404 / 1886 / 201 / 40 / 55 / 7 / 28**. **FOUR of the seven move and THREE must not**: shell
1886 -> 1915, imgui 201 -> 209, inspector 55 -> 59 and meta 7 -> 10 all move, while `aero_tests` (1404)
says no engine behaviour changed and `aero_scene_serialize_test` (40) and `aero_reflect_json_test` (28)
say no serialized and no JSON change crept in — an annotation is invisible to `--emit-json` by
construction — and a moved one of those three is a stop-and-find-out. Guards
**516 / 92 / 163 / 92 / 165 / A=6 B=86 / 11-3-55 / 6-57**: math and Check B move by E.3.3's ten new
tracked C-family files and four new `editor/src/*.cpp`, and the other six are unmoved from E.3.1's
**506 / 92 / 163 / 92 / 165 / A=6 B=82 / 11-3-55 / 6-57**. **A THIRD guard moving is a
stop-and-find-out.** The E.3.2 figures immediately below are that task's own gate and are superseded by
these.

**A TASK CAN MOVE `ctest -N` WITHOUT ADDING A COMPONENT OR A TARGET.** E.3.3's four additions are
`reflect-gen.*` **process** cases inside `if(AERO_REFLECT_TOOLS)`, so they are tools-ON only — read a
`reflect-gen.*` addition and a `cooker.*` addition as DIFFERENT kinds of move, because a `cooker.*` one
must be identical in all three configurations and no test can report that.
...
The eight guard counts at **E.3.3's** gate: math **516**, platform **92**, rhi **163**, scene **92**,
golden-rule **165**, project-no-delete **A=6/B=86**, audio **11/3/55**, probes **6/57** — math and
project-no-delete moved by E.3.3's ten new tracked C-family files (eight editor, two tests), and the
other six are unmoved from E.3.2's **506 / 92 / 163 / 92 / 165 / A=6 B=82 / 11-3-55 / 6-57**. **A THIRD
guard moving on a task of this shape is a stop-and-find-out.** **The two
reduced configurations read `165` (shader-tools-OFF) and `93` (reflect-tools-OFF)**, both re-measured
...
`173 - 13 = 160` at E.2.1's gate, `174 - 13 = 161` at E.3.2's and is `178 - 13 = 165` now — the number
moves with EVERY `reflect-gen.*` addition, because those are `AERO_REFLECT_TOOLS`-gated and therefore
present in this configuration. 3.7.3's remembered `80 / 93` is HALF WRONG for the same species of reason. Compare the
...
it said 75 + 4 = 79 removals, at E.2.3's gate it was 77 + 4 = 81, and measured at E.3.3's gate it is
**81 + 4 = 85** (178 - 85 = 93) — E.2.1 and E.2.2 each added a per-header `reflect-gen.components_*`
entry and E.3.3 added four `reflect-gen.asset_*` ones, all after that sentence was first written.
**The TOTAL 93 has not moved through any of it, and the breakdown has moved three times — re-measure
both, every time.**
...
**TWO PAGES HAVE NOT BEEN RUN ON ANY PLATFORM: E.1.5's and E.3.2's. E.3.3's IS macOS-RUN -- 10 PASS / 3 PARTIAL / 1 NOT EXECUTABLE / 1 NOT RUN, 2026-09-21, nothing failed.** **E.3.3's is fifteen
rows** (`editor/validation/E.3.3-asset-reference-picker.md`, gitignored, so it enters no commit), and
**SEVEN of them are the ONLY cover something has anywhere** — six declared sabotage seeds plus row 15,
which the code-review round added for the two draw-list rules (a `##` in an asset's name, and a tile
face aligned with its tile), because **both of those seeds come back green at every automated tier**: row 1 for `S45` (the value
button's width against `Clear` — nothing here measures a widget rect), row 12 for `S42` (one WARN per
OPEN rather than per frame — no GPU-tier component carries an unknown token, and annotating a built-in
to make a test possible is a worse trade), row 6 for `S27` and AC-11's audio half (**no tier in this
tree can drag**), rows 3 and 5 for `S28`–`S32` (the click-to-commit close and the whole keyboard, whose
SPELLINGS `I166(b)` pins and whose BEHAVIOUR it cannot), and row 7's truncation half for `S24`'s visible
consequence. E.3.2's is twelve rows
...
**Phase E is the open front, and EPIC E.3 IS THREE QUARTERS DONE: E.3.1, E.3.2 and E.3.3 are all
merged; only E.3.4 is planning only.** **E.3.3 (asset-reference picker) is the newest — MERGED as PR #104, merge commit `fc77c4b`, sixteen
commits, the full local gate green on both presets and both reduced configurations, the 46-seed
sabotage matrix run in full, and a code-review round that found FOUR should-fix gaps, none blocking.**
It moved `ctest -N` for the first time
since 3.7.3 (**174 -> 178**, four `reflect-gen.asset_*` process cases) and took the ImGui-tier case-id
ceiling to **`I169`**; `I162`–`I169` are claimed, so **`I170` is the next free id**. **IT IS macOS-VALIDATED — 10 PASS / 3 PARTIAL / 1 NOT EXECUTABLE / 1 NOT RUN, 2026-09-21, 43 of 51
records ticked and NOTHING FAILED**, which closes the two draw-list rules the review round added: the
button reads a `##` name WHOLE, and the tile face is centred identically in BOTH hosts (22/23/14/73
device px in each). **The culled-grid defect is absent in the product** at 330 pt against a ~415 pt
popup. The three PARTIALs and the NOT EXECUTABLE are input limits, not defects — nothing here can type
or drag, and only a 2x display was attached; **row 12 (S42) and row 14 (cost) are NOT RUN**, and
Windows and Linux are unrun as everywhere.** The matrix found **four holes in its own tests** and,
chasing two of them, **a real product defect** — an API-positioned popup taller than the work area ran
off the bottom of the screen and ImGui CULLED its grid child, which submits no tiles and says nothing.
**E.3.1's `I142`–`I148` case-id block is discharged and the numbering is contiguous again.** **`editor_prefs.json` NOW EXISTS BY NAME**, with its own
...
**E.3.2 and E.3.3 add none and close none**. E.3.3 adds a THIRD gap of its own shape, already owned:
**a `meshIndex`-aware model pick** (picking a model writes `mesh` alone; a stale index draws nothing and
is counted, which is 3.1.5's own degradation).
...
**The spine, as E.3.3 leaves it. Epics E.3 (ONE task left), E.4, E.5 and E.6 are the open
front — twelve tasks, planning only** (24 in the phase, 12 merged: E.1's five, E.2's four, E.3.1, E.3.2
and E.3.3). **E.4.5 (thumbnails) is unblocked TWICE OVER now**: it has `material_preview_rig.hpp` to
call BY NAME — a thumbnail is `materialPreviewCamera(rig, fixedAngle, 1.0F)` plus
`materialPreviewView(...)` with whatever `MaterialPreviewLighting` it wants — **and it has the
`ThumbnailService` E.3.3 built, which is the "a second PRODUCER, not a second cache" home**: a rendered
material thumbnail marks a key `Ready` through the same ledger, is evicted by the same LRU and is drawn
by the same `drawAssetTileFace`. **`thumbnailKeyForRecord`'s decodable-extension guard is the ONE line
that producer widens**, and it is a pure function with tier-0 cover now, which it was not.
**E.3.4 inherits the widget itself**: it restyles the slot ROW around `drawAssetReferenceField` — a
thumbnail beside it, the colour-space note, the clear action's look — and **must not build a second
picker**; both pointers it needs are already lent and `drawSlotSection` is already a member, so the
row's layout is one function's body. **E.3 still inherits exactly TWO Inspector-row gaps** — E.2.1's enum-aware row
...
**EIGHT UNOWNED HANDOFFS, THREE OF THEM NEW AT E.3.3.** The three new ones: **a `meshIndex`-aware
model pick** (a picker that chooses `(mesh, meshIndex)` together needs either a reflectable "sub-asset"
annotation or a component-aware row, and both are a design; likely first customer 4.7 or E.5.x);
**`AERO_ASSET(text)` and a `Text` reference** — the vocabulary IS the draggable set, so the day a script
component wants a `.json` it is ONE edit to `assetKindIsDraggable` and three tests (`DR10`, `AR5`,
`MP1`); and **a "reveal in browser" verb** from the picker or the row, which is E.3.2's file-open handoff
seen from the other side and needs the same new `ActionKind`. **AND THE FIVE CARRIED:** **Spot and point shadows**: the shadow pass is
