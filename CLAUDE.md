...
**3.4.2 (Material inspector editing) is COMPLETE IN CODE on `feat/3.4.2-material-inspector-editing`
(tip `2ac77b2`, 14 commits), local gate fully green, NOT pushed and NOT merged; its twelve-row macOS
validation page is written and its pass is the next thing that runs.** Rows 3, 4 and 6 of that page
are the only coverage six of **seven** declared sabotage seeds have anywhere; the seventh, S26, is
observable on no Apple platform at all and waits on a Windows or Linux pass.
...
Test inventory at the tip of `feat/3.4.2-material-inspector-editing` (`2ac77b2`), every number
