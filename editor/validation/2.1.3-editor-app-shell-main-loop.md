...
2. **Delete `build/<os>-debug/editor/aero_editor.ini`** (it sits next to the exe), then run
   `aero_editor`.
3. **Menu bar**: exactly three menus — File, Edit, View. Open each. **[Corrected by task 2.5.1 — the
   recorded macOS PASS below predates this instruction change and is not invalidated by it.]** `File`
   now has New Scene / Open Scene... / Save Scene / Save Scene As... **all LIVE** (Save Scene greyed
   only when the document is clean and titled, AC-4; the other three greyed only while a native dialog
   is in flight, D8, or when the editor was built with `AERO_REFLECT_TOOLS=OFF`, D18) / separator /
   **Exit** (also live, routed through the same unsaved-changes guard, D1); `Edit` has Undo / Redo
   (both greyed on a fresh, unedited document); `View` lists all five panels with checkboxes, a
   separator, then Reset Layout.
4. **No disabled stub remains anywhere in the editor** — task 2.5.1 deleted the last one
   (`menuItemStub`, the File-menu placeholders). There is nothing left to hover for an "owned by task
   N" tooltip; the only tooltips left in the File menu name `AERO_REFLECT_TOOLS`, not an owning task
   (see task 2.5.1's own validation page).
