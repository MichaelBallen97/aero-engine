...
// ---- Group 4: refusals and hostile input (AD12-AD21) ---------------------------------------------
...
TEST_CASE("AD21: a LYING length is caught by the in-loop check, which nothing else here can reach") {
    // A28's witness, and the one arm in this file whose input is corrupt on purpose.
    // tone-lying-length.ogg is tone.ogg with its last page's granule position rewritten from 8000 to
    // 10 and that page's framing CRC recomputed -- six bytes, generated by a script recorded verbatim
    // in tests/fixtures/audio/README.md, never a hand edit.
    //
    // Why the ogg and not an mp3, MEASURED rather than assumed: dr_mp3 does trust a Xing/Info frame
    // count unclamped when it REPORTS a length, but ma_dr_mp3_read_pcm_frames_raw bounds its own
    // reads by that same count, so a low lie makes the decode SHORTER (16 -> 4 gives 1088 frames, not
    // 8000) and a high lie is refused before the loop is reached. dr_wav is the same -- a hand-built
    // Wave64 claiming 10 frames over 8000 frames of data decodes 10. stb_vorbis is the one backend
    // whose decode loop never consults its own total_samples, which is what makes this reachable.
    const DecodedAudio honest = decodeFixture("tone.ogg");
    REQUIRE(honest.error.empty());
    REQUIRE(honest.channels == 2);
    const std::size_t honestFrames = honest.samples.size() / honest.channels;

    // 1. The lie is real: the stream still decodes far more than its header claims, and the decode is
    //    not simply identical to the honest file either -- the LAST page is truncated to its own
    //    granule position, so this is 7168 rather than 8000.
    const DecodedAudio lying = decodeFixture("tone-lying-length.ogg");
    REQUIRE(lying.error.empty());
    REQUIRE(lying.channels == 2);
    const std::size_t lyingFrames = lying.samples.size() / lying.channels;
    MESSAGE("AD21: the header claims 10 frames; the stream decoded " << lyingFrames);
    CHECK(lyingFrames > 10);
    CHECK(lyingFrames < honestFrames);

    // 2. THE POINT. Both files are refused at the same small cap -- but only the WORDING says which
    //    check did it, so a status-only assertion cannot see the in-loop half at all. The honest file
    //    is stopped before anything is reserved, naming its own true frame count; the lying one gets
    //    past that check on a claim of 10 and is stopped by the loop.
    constexpr std::uint32_t SMALL_CAP = 100;
    const DecodedAudio honestCapped = decodeFixture("tone.ogg", SMALL_CAP);
    CHECK(!honestCapped.error.empty());
    CHECK(honestCapped.error.find("declares " + std::to_string(honestFrames) + " frames") != std::string::npos);

    const DecodedAudio lyingCapped = decodeFixture("tone-lying-length.ogg", SMALL_CAP);
    CHECK(!lyingCapped.error.empty());
    CHECK(lyingCapped.samples.empty());
    MESSAGE("AD21: lying + cap message: " << lyingCapped.error);
    CHECK(lyingCapped.error.find("decodes to more than 100 frames") != std::string::npos);
    CHECK(lyingCapped.error.find("declares") == std::string::npos);

    // 3. And the same for the PRODUCT bound, whose in-loop half was equally unwitnessed. At two
    //    channels a sample cap of 200 is the frame cap of 100 above, so the frame cap alone cannot
    //    account for this refusal.
    const DecodedAudio lyingSamples = decodeFixture("tone-lying-length.ogg", GENEROUS_FRAMES, GENEROUS_CHANNELS, 200);
    CHECK(!lyingSamples.error.empty());
    CHECK(lyingSamples.samples.empty());
    MESSAGE("AD21: lying + sample cap message: " << lyingSamples.error);
    CHECK(lyingSamples.error.find("decodes to more than 200 samples") != std::string::npos);
    CHECK(lyingSamples.error.find("declares") == std::string::npos);
}
