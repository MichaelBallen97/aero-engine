...
### Implementation note (task 2.2.2)

The inspector consumer landed: the editor now walks generated `entt::meta` directly and renders an
editing UI for **any** `[[engine::component]]`-annotated type with **zero per-component editor code**
— proven by a fixture (`InspectorProbe`) no editor source names. The annotation vocabulary grew two
field-level markers, `AERO_RANGE(min, max)` and `AERO_COLOR`, mirrored at runtime by
`engine::reflect::FieldUiMeta`, carried as **sparse entt custom data** attached per data member (an
unannotated field carries none — `.custom<>()` returns `nullptr`, and the conditional
`#include <aero/reflect/annotations.hpp>` in generated code is emitted only when at least one custom
will exist, so an annotation-free consumer's generated bytes are unchanged). `std::string` joined the
reflectable subset end-to-end (detection, `--emit-meta`, `--emit-json`, `serialize.hpp`) via three
exact canonical-spelling comparisons — `std::string`, `std::basic_string<char>`, and the fully
belt-and-braces default-allocator spelling — never a prefix match, which would also accept an
allocator-customized `std::basic_string` `serialize.hpp` has no overload for.

`engine::editor::component_ops` (`editor/include/aero/editor/component_ops.hpp`) is the seam
task 2.4.2 will **wrap, not rewrite**: four entt-free functions
(`addComponent`/`removeComponent`/`readComponentField`/`writeComponentField`) whose constness is
**deliberately not uniform** — `buildInspectorModel` (`inspector_model.hpp`) takes `const World&`
because its read path (`World::getRaw() const` → `entt::meta_type::from_void(const void*)` → a
by-value-copied `meta_any`) provably needs nothing more, making the model build's non-mutation a
compiler fact; the seam's own four functions take `World&` because the write path
(`from_void(void*)` + `meta_data::set`) genuinely refuses a const instance. The seam narrows and
clamps in C++ before writing the exact concrete type, rather than letting EnTT convert — EnTT's own
`set()` wraps an out-of-range value into a narrower integer (measured: `300` into a `uint8_t` becomes
`44`) and silently accepts `bool`↔numeric crossings, neither of which the seam permits.

The renderable arithmetic subset is **17 C++ types**, not the 15 a naive reading of the reflectable
range might suggest: `tools/reflect-gen`'s 18-`CXTypeKind` whitelist (task 1.2.2) maps to 17 distinct
C++ types (`Char_U`/`Char_S` both denote plain `char`), and the inspector's own type map
(`editor/src/meta_utils.hpp`) is kept in lock-step with it on purpose — omitting any of `char`,
`signed char`, `char16_t`, `char32_t`, `wchar_t`, `long`, or `unsigned long` would silently accept such
a field into `entt::meta` while rendering nothing for it in the UI, with no warning anywhere. A
defensive, loud `AERO_LOG_ERROR` fires if the two lists ever drift apart regardless.

### Implementation note (task E.3.3)

**The fourth field annotation.** `AERO_ASSET(kind)` joins `AERO_RANGE`/`AERO_COLOR`, mirrored at runtime
by `FieldUiMeta::assetKind` — a `const char*` **appended last**, under that struct's frozen-order
contract, pointing at a string literal in the generated TU. It is **sparse by construction**: the emitter
writes `.assetKind = "…"` only when the annotation is present, so an unannotated field's generated bytes
are *identical* to what they were before the member existed, which `reflect-gen.annotations_meta` and
`reflect-gen.guid_meta` prove by staying green **unedited**. Applicability is judged after
classification, exactly as `engine::range` and `engine::color` already are: on a non-`Guid` field it
warns and is dropped, never an error.

**The vocabulary split, which is the part worth recording.** The tool validates the token's **grammar**
(it is an identifier) and passes it through; the **editor** owns the **vocabulary**, because the set of
kinds is `AssetKind`, an editor type the engine's reflect layer must not know — the golden rule runs the
other way. A token the editor does not recognise is *unconstrained plus one warning*, never an error,
which is `AERO_RANGE`'s own leniency one annotation over. **The reflectable subset does not grow**: this
