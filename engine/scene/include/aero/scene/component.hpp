...
// One byte per component type, at a unique address. The member is ANCHOR, not anchor, because
// .clang-tidy's readability-identifier-naming.ClassConstantCase is UPPER_CASE.
template <typename T>
...
template <typename T>
const char ComponentTypeTag<T>::ANCHOR = 0;
